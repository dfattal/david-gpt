...
# This script systematically tests all 21 questions from the RAG-TESTING-GUIDE.md file.

# --- Configuration ---
API_URL="http://localhost:3000/api/chat"
REPORT_FILE="rag-test-results.md"
TIMEOUT_SECONDS=60
...
QUESTIONS=(
    # Section 1: Products Integrating Leia Display Technology
    "What were the key selling points of the RED Hydrogen One smartphone's display?"
    ...
    # Section 2: LIF Files (Leia Image Format)
    "What is a LIF file and how does it store 3D image data?"
    ...
    # Section 3: Evolution of Leia Technology
    "What was Diffractive Lightfield Backlighting (DLB) and what were its main limitations?"
...
send_request() {
    ...
    json_payload=$(jq -n \
        ...
        --arg persona "$PERSONA" \
        '{message: $question, conversationId: $convId, persona: $persona, stream: false}')

    # Make the API call with timeout and capture HTTP status
...
echo "# RAG System Test Results" > "${REPORT_FILE}"
