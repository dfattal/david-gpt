...
1. Use EXACT field names from INGESTION-FORMAT.md: "docType" (not "doc_type"), "authorsAffiliations" (not "author")
2. Use inline array format: ["item1", "item2"] NOT block format
3. Include ALL required fields: title, docType, authorsAffiliations, venue, publicationYear, abstract, keywords, technologies, url, scraped_at, word_count, extraction_quality
4. Use "${new Date().toISOString()}" for scraped_at timestamp
5. Use structured authorsAffiliations format:
   authorsAffiliations:
     - name: "Author Name"
...
1. Create complete YAML frontmatter following INGESTION-FORMAT.md with:
   - title: "${metadata.title}"
   - docType: "paper"
   - authorsAffiliations: (structured format with name and affiliation for each author)
   - venue: "arXiv"
...
IMPORTANT: Return ONLY the raw markdown content starting with YAML frontmatter (---). Do NOT wrap in code blocks or add any markdown formatting markers.`;
  ...
  private generateYAMLFrontmatter(document: DocumentEntry, extractionResult?: any): string {
    ...
    if (docType === 'press-article') {
      ...
      // Use structured authorsAffiliations format as required by INGESTION-FORMAT.md
      const authors = extractionResult?.authors || extractionResult?.author?.split(', ') || ['Unknown Author'];
      frontmatter.push(
        `authorsAffiliations:`
      );
      ...
      frontmatter.push(
        `author: "${extractionResult?.author || 'David Fattal'}"`,
...
CRITICAL FORMAT REQUIREMENTS:
1. Use EXACT field names: "docType" (not "doc_type"), "authorsAffiliations" (not "author")
2. Use inline array format: ["item1", "item2"] NOT block format
...
IMPORTANT: Return ONLY the raw markdown content starting with YAML frontmatter (---). Do NOT wrap in code blocks or add any markdown formatting markers.`;
  ...
  private validatePaperFormat(content: string): { isValid: boolean; reason?: string } {
    const requiredFields = [
      'title:', 'docType:', 'authorsAffiliations:', 'venue:',
    ...
    if (content.includes('authors:') && !content.includes('authorsAffiliations:')) {
      return { isValid: false, reason: 'Found "authors:" instead of required "authorsAffiliations:"' };
    }
