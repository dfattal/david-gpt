const fs = require('fs');

async function testCAT3DUpload() {
  try {
    console.log('🚀 Testing CAT3D.pdf batch API upload...\n');
    
    // Read the CAT3D.pdf file
    const fileContent = fs.readFileSync('./RAG-SAMPLES/CAT3D.pdf');
    
    // Create form data
    const formData = new FormData();
    const blob = new Blob([fileContent], { type: 'application/pdf' });
    const file = new File([blob], 'CAT3D.pdf', { type: 'application/pdf' });
    
    formData.append('files', file);
    formData.append('description', 'Testing CAT3D.pdf chunking - investigating 5418 vs expected 15-20 chunks');
    
    const cookies = 'sb-mnjrwjtzfjfixdjrerke-auth-token.0=base64-%3D%3D; sb-mnjrwjtzfjfixdjrerke-auth-token.1=';
    
    const response = await fetch('http://localhost:3001/api/documents/batch-ingest', {
      method: 'POST',
      headers: {
        'Cookie': cookies
      },
      body: formData
    });
    
    console.log('Response Status:', response.status);
    console.log('Response Headers:', Object.fromEntries(response.headers.entries()));
    
    const responseText = await response.text();
    console.log('Response Text (first 500 chars):', responseText.substring(0, 500));
    
    let result;
    try {
      result = JSON.parse(responseText);
      console.log('Parsed JSON:', JSON.stringify(result, null, 2));
    } catch (parseError) {
      console.log('Failed to parse JSON, response was HTML or other format');
      return;
    }
    
    if (result.batchId) {
      console.log('\n📊 Monitoring processing status...');
      
      // Monitor progress
      let completed = false;
      let attempts = 0;
      while (!completed && attempts < 60) { // Extended timeout for large PDF
        await new Promise(resolve => setTimeout(resolve, 5000)); // 5 second intervals
        
        const progressResponse = await fetch(`http://localhost:3001/api/ingestion/progress?batchId=${result.batchId}`, {
          headers: { 'Cookie': cookies }
        });
        
        const progress = await progressResponse.json();
        console.log(`Progress: ${progress.completed}/${progress.total} - Status: ${progress.status}`);
        
        if (progress.details && progress.details.length > 0) {
          const detail = progress.details[0];
          if (detail.chunkCount !== undefined) {
            console.log(`📄 Document: "${detail.title}" - Chunks: ${detail.chunkCount}`);
          }
        }
        
        if (progress.status === 'completed') {
          completed = true;
          console.log('\n✅ Processing completed! Final results:');
          console.log(JSON.stringify(progress, null, 2));
        } else if (progress.status === 'failed') {
          completed = true;
          console.log('\n❌ Processing failed!');
          console.log(JSON.stringify(progress, null, 2));
        }
        
        attempts++;
      }
      
      if (!completed) {
        console.log('\n⏰ Timeout reached. Final status check...');
        const finalResponse = await fetch(`http://localhost:3001/api/ingestion/progress?batchId=${result.batchId}`, {
          headers: { 'Cookie': cookies }
        });
        const finalProgress = await finalResponse.json();
        console.log(JSON.stringify(finalProgress, null, 2));
      }
    }
    
  } catch (error) {
    console.error('❌ Error:', error);
  }
}

testCAT3DUpload();