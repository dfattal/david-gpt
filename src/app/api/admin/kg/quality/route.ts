...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: entityStats } = await supabaseAdmin
      ...
      .select('kind')
      .then(result => ({
        data: result.data?.reduce((acc: any, entity) => {
          if (entity.kind) {
    ...
    try {
      ...
      orphanedEntities = fallbackResult.data?.filter(entity =>
    ...
    const duplicates: Array<{
      entities: Array<{ id: string; name: string; kind: string; authority_score: number }>;
      similarity: number;
    }> = [];
    ...
    const { count: entitiesWithoutAliases } = await supabaseAdmin
      ...
      .not('id', 'in', 
        `(${supabaseAdmin.from('aliases').select('entity_id').then(r => r.data?.map(a => a.entity_id).join(',') || '')})`
      );
    ...
    const { data: relationshipStats } = await supabaseAdmin
      ...
      .select('rel')
      .then(result => ({
        data: result.data?.reduce((acc: any, edge) => {
    ...
    const qualityReport = {
      ...
      recommendations: [
        ...(duplicates.length > 0 ? ['Consider merging potential duplicate entities'] : []),
        ...(orphanedEntities && orphanedEntities.length > 0 ? ['Review orphaned entities for relevance'] : []),
        ...((lowAuthorityEntities?.length || 0) > 0 ? ['Validate or remove low authority entities'] : []),
        ...((weakRelationships?.length || 0) > 0 ? ['Review and strengthen weak relationships'] : []),
      ]
    };
