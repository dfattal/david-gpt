...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tables = [
      'search_queries', 'processing_jobs', 'message_citations', 'messages',
      'conversation_sources', 'conversations', 'aliases', 'events', 
      'edges', 'entities', 'document_chunks', 'documents'
    ];
    ...
    const deletionResults: Record<string, { success: boolean; error?: string; deletedCount?: number }> = {};
    ...
    for (const table of deletionOrder) {
      try {
        ...
        console.log(`✅ Deleted ${initialCounts[table] || 0} records from ${table}`);
    ...
    const successCount = Object.values(deletionResults).filter(result => result.success).length;
    const errorCount = Object.values(deletionResults).filter(result => !result.success).length;
