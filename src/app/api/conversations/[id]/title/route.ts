...
export async function POST(
  ...
  console.log(`[Title Generation API] Starting title generation for conversation ${id}`)
  ...
  try {
    ...
    // ID already extracted above for logging
    ...
    if (!messages || messages.length === 0) {
      console.warn(`[Title Generation API] No messages found for conversation ${id}`)
      return Response.json({ error: 'No messages found for title generation' }, { status: 400 })
    }
    
    console.log(`[Title Generation API] Found ${messages.length} messages for conversation ${id}`)
    ...
    console.log(`[Title Generation API] Generating title for conversation ${id} with context: ${conversationContext.substring(0, 200)}...`)
    ...
    console.log(`[Title Generation API] Raw generated title: "${generatedTitle}"`)
    ...
    console.log(`[Title Generation API] Clean title: "${generatedTitle}" (${wordCount} words)`)
    
    if (wordCount < 2 || wordCount > 8 || generatedTitle.length < 5) {
      console.warn(`[Title Generation API] Title validation failed: wordCount=${wordCount}, length=${generatedTitle.length}. Using fallback.`)
    ...
    if (updateError) {
      console.error(`[Title Generation API] Failed to update conversation title for ${id}:`, updateError)
      
      // Check if it's because title_status is no longer 'pending' (race condition)
      if (updateError.code === 'PGRST116') {
        console.log(`[Title Generation API] Title already generated by another process for ${id}`)
        return Response.json({ message: 'Title already generated' }, { status: 200 })
      }
      
      // Mark title generation as failed only if still pending
      await supabase
        .from('conversations')
    ...
    console.log(`[Title Generation API] Successfully generated and saved title for conversation ${id}: "${generatedTitle}"`)
    ...
    try {
      ...
      await supabaseError
        ...
        .eq('id', errorId)
        .eq('title_status', 'pending') // Only update if still pending
    } catch (cleanupError) {
