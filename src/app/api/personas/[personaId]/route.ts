...
export async function GET(
  ...
  try {
    ...
    console.error(
      `Failed to get persona ${(await context.params).personaId}:`,
      error
    );
  ...
  try {
    ...
    if (updates.persona_md || updates.constraints_yaml) {
      ...
      const contentToValidate = {
        ...
        constraintsYaml:
          updates.constraints_yaml || currentPersona.constraints_yaml,
      };

      const contentValidation =
        PersonaValidator.validateFromContent(contentToValidate);
    ...
    // Update persona
    const updatedPersona = await PersonaManager.updatePersona(
      personaId,
      updates
    );
    ...
    console.error(
      `Failed to update persona ${(await context.params).personaId}:`,
      error
    );
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    console.error(
      `Failed to delete persona ${(await context.params).personaId}:`,
      error
    );
    return NextResponse.json(
