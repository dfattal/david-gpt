...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!contentType.includes('multipart/form-data')) {
      throw new AppError('Folder upload requires multipart/form-data with files', 400);
    }
    ...
    const batchDescription = formData.get('batchDescription') as string || 'Folder batch ingestion';
    ...
    if (files.length === 0) {
      throw new AppError('No markdown files found in upload. Please ensure your folder contains .md or .markdown files.', 400);
    }
    ...
    for (const file of files) {
      try {
        ...
        const validation = DocumentFormatValidator.validateDocument(file.content, file.name);
        ...
        const folderPath = file.path.includes('/')
          ...
          : 'root';
        validationSummary.folderStructure[folderPath] = (validationSummary.folderStructure[folderPath] || 0) + 1;

        // Extract frontmatter for document type
        const { frontmatter } = parseFrontmatter(file.content);
        const docType = frontmatter.docType || 'note';
        validationSummary.documentTypes[docType] = (validationSummary.documentTypes[docType] || 0) + 1;

        // Create processed document entry
        const processedDoc: ProcessedDocument = {
          title: frontmatter.title || extractTitleFromContent(file.content) || file.name.replace(/\.md$/, ''),
          filePath: file.path,
          folderPath,
        ...
        if (!skipInvalidFiles) {
          ...
          processedDocuments.push({
            ...
            filePath: file.path,
            folderPath: file.path.includes('/') ? file.path.substring(0, file.path.lastIndexOf('/')) : 'root',
    ...
    const documentsToIngest = strictValidation
      ? processedDocuments.filter(doc => doc.validation?.isValid && doc.validation.warnings.length === 0)
      : processedDocuments.filter(doc => doc.validation?.isValid || !skipInvalidFiles);
  ...
  try {
    ...
    if (action === 'folder-structure-example') {
      return NextResponse.json({
        ...
        example: {
          'my-corpus/papers/ai-research-2024.md': 'Research paper with docType: "paper"',
          'my-corpus/patents/display-tech-patent.md': 'Patent with docType: "patent"',
          'my-corpus/articles/tech-news.md': 'Article with docType: "press-article"',
          'my-corpus/notes/meeting-notes.md': 'Personal note with docType: "note"'
        }
      });
    }
    ...
    return NextResponse.json({
      ...
      methods: ['POST', 'GET'],
      description: 'Folder-based batch ingestion with recursive processing for /my-corpus structure',
      usage: {
        POST: {
      ...
      supportedActions: {
        'folder-structure-example': 'Get recommended /my-corpus folder organization',
        'validation-requirements': 'Get validation requirements and quality scoring details'
      }
    });
