  ...
  try {
    ...
    if (!personaManager.isEnhanced(persona as Persona)) {
      try {
        ...
        console.warn(
          `Failed to load enhanced persona ${persona}, using fallback:`,
          error
        );
      }
    }
    ...
    const baseSystemPrompt = personaManager.generateSystemPrompt(
      persona as Persona
    );
...
Always provide accurate, helpful responses with transparent sourcing while maintaining proper markdown formatting for excellent readability.`;
    ...
    if (useRAG) {
      ...
      console.log(
        `🔄 Context Management: ${ragContext.contextUsed ? 'ACTIVE' : 'DISABLED'}`
      );
      if (ragContext.contextUsed && ragContext.turnAnalysis) {
        console.log(
          `🎯 Turn: ${ragContext.turnAnalysis.turnType}, Mode: ${ragContext.turnAnalysis.responseMode}`
        );
        console.log(
          `📈 Sources: ${ragContext.freshResults} fresh + ${ragContext.carryOverResults} carried`
        );
      }
    }
      ...
      if (user && completion.text) {
        try {
          ...
          if (!actualConversationId) {
            console.log(
              '🔍 No conversationId provided, searching for recently created conversation...'
            );
            const { data: recentConversations, error: searchError } =
              await supabase
                .from('conversations')
            ...
            if (!searchError && recentConversations?.length > 0) {
              ...
              const recentConv = recentConversations.find(
                (conv: { id: string; created_at: string }) =>
                  conv.created_at > thirtySecondsAgo
              );

              if (recentConv) {
                actualConversationId = recentConv.id;
                console.log(
                  `✅ Found recent conversation: ${actualConversationId}`
                );
              }
            }
          }
          ...
          if (!actualConversationId) {
            console.log(
              '⚠️ No conversation ID available, skipping message save'
            );
            return;
          }
          ...
          console.log(
            `⚡ Parallel database operations completed in ${operationTime.toFixed(0)}ms`
          );
          ...
          if (
            existingMessageCheck.status === 'fulfilled' &&
            !existingMessageCheck.value.error
          ) {
            userMessageExists =
              (existingMessageCheck.value.data?.length || 0) > 0;
            if (userMessageExists) {
              console.log(
                '✅ User message already exists, skipping duplicate save'
              );
            }
          }
          ...
          if (
            ...
            import('@/lib/performance/batch-citations')
              ...
              .then(() =>
                console.log('✅ Citations queued for batch processing')
              )
              .catch(error =>
...
async function handleCitationPersistence(
  ...
  try {
    ...
    if (
      ragContext.enhancedCitations &&
      ragContext.enhancedCitations.length > 0
    ) {
      console.log(
        `💾 Persisting ${ragContext.enhancedCitations.length} enhanced citations...`
      );
