...
export async function testCohereRerank() {
  ...
  try {
    ...
    console.log(
      `📊 Search results: ${searchResult.results.length} documents found`
    );
    ...
    if (searchResult.results.length > 0) {
      ...
      const hasRerankedScores = searchResult.results.some(
        r => r.rerankedScore !== undefined
      );
      if (hasRerankedScores) {
        console.log('🎯 Reranking was successfully applied!');
      } else {
        console.log(
          '⚠️  No reranked scores found - may indicate reranking was not applied'
        );
      }
    } else {
    ...
    return {
      ...
      rerankingApplied: searchResult.results.some(
        r => r.rerankedScore !== undefined
      ),
