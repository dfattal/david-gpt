...
export const CORE_TECHNOLOGIES: TechnologyDefinition[] = [
  {
    ...
    description: 'Display technology that can dynamically switch between high-resolution 2D mode and autostereoscopic 3D mode, addressing the resolution trade-off in traditional 3D displays.',
    keyInnovations: [
  ...
  },
  {
    ...
    category: 'display_technology', 
    description: 'Advanced 3D display technology that tracks viewer eye position and dynamically adjusts rendered views for precise stereoscopic projection to each eye.',
    keyInnovations: [
  ...
  },
  {
    ...
    category: 'display_technology',
    description: 'Leia\'s original lightfield display approach using diffractive optical elements integrated into the backlight to create directional light fields for autostereoscopic viewing.',
    keyInnovations: [
  ...
  },
  {
    ...
    category: 'display_technology',
    description: 'Traditional autostereoscopic technology using a physical barrier with precise apertures to separate left and right eye views.',
    keyInnovations: [
...
export const CORE_COMPONENTS: ComponentDefinition[] = [
  {
    ...
    type: 'optical',
    description: 'Liquid crystal cell that can be electrically switched to enable or disable lenticular focusing, enabling dynamic 2D/3D mode switching.',
    function: 'Electrically controllable optical switching between transparent and lenticular states',
    compatibleTechnologies: ['switchable-2d3d-display', 'eye-tracked-stereoscopic'],
  ...
  },
  {
    ...
    type: 'optical',
    description: 'Backlight unit with integrated diffractive optical elements that create directional light fields for autostereoscopic displays.',
    function: 'Generate multiple directional light cones for different viewing zones',
  ...
  },
  {
    ...
    type: 'electrical',
    description: 'Camera-based sensor system that tracks viewer eye position in real-time for gaze-aware displays.',
    function: 'Real-time detection and tracking of viewer eye positions and gaze direction',
  ...
  },
  {
    ...
    type: 'optical',
    description: 'Physical barrier layer with precisely positioned apertures for traditional autostereoscopic displays.',
    function: 'Spatial separation of left and right eye images through occlusion',
 ...
 */
export function getTechnologyInfo(technologyId: string): TechnologyDefinition | null {
 ...
 */
export function getComponentInfo(componentId: string): ComponentDefinition | null {
 ...
 */
export function findTechnologyByName(name: string): TechnologyDefinition | null {
  const normalizedName = name.toLowerCase().trim();
  
  return CORE_TECHNOLOGIES.find(tech => {
    const techName = tech.name.toLowerCase();
    return techName.includes(normalizedName) || normalizedName.includes(techName) ||
           // Check for key terms
           (normalizedName.includes('switchable') && techName.includes('switchable')) ||
           (normalizedName.includes('eye') && normalizedName.includes('track') && techName.includes('eye-tracked')) ||
           (normalizedName.includes('diffractive') && techName.includes('diffractive')) ||
...
export function findComponentByName(name: string): ComponentDefinition | null {
  ...
  return CORE_COMPONENTS.find(comp => {
    const compName = comp.name.toLowerCase();
    return compName.includes(normalizedName) || normalizedName.includes(compName) ||
           // Check for key terms
           (normalizedName.includes('lc') && compName.includes('lc')) ||
           (normalizedName.includes('liquid crystal') && compName.includes('lc')) ||
           (normalizedName.includes('eye') && normalizedName.includes('track') && compName.includes('eye')) ||
           (normalizedName.includes('diffractive') && compName.includes('diffractive')) ||
...
export function getTechnologyEvolution(technologyId: string): {
  ...
  const predecessors = TECHNOLOGY_EVOLUTION_TIMELINE.filter(evo => evo.toTech === technologyId);
  const successors = TECHNOLOGY_EVOLUTION_TIMELINE.filter(evo => evo.fromTech === technologyId);
 ...
 */
export function getCompatibleTechnologies(componentId: string): TechnologyDefinition[] {
 ...
 */
export function getTechnologyComponents(technologyId: string): ComponentDefinition[] {
...
export function generateRelationshipSummary(
  ...
  if (tech1 && tech2 && relationshipType === 'evolved_to') {
    const evolution = TECHNOLOGY_EVOLUTION_TIMELINE.find(evo => 
      evo.fromTech === tech1.id && evo.toTech === tech2.id
    );
    if (evolution) {
      return `${tech1.name} evolved to ${tech2.name} during ${evolution.timeframe}. ` +
  ...
  if (tech1 && comp2 && relationshipType === 'can_use') {
    if (comp2.compatibleTechnologies.includes(tech1.id)) {
      return `${tech1.name} can use ${comp2.name}. ${comp2.description}. ` +
  ...
  if (comp1 && comp2 && relationshipType === 'alternative_to') {
    ...
    if (commonTechs.length > 0) {
      return `${comp1.name} and ${comp2.name} are alternative components for ` +
...
export function getCuratedKnowledgeContext(query: string): {
  ...
  const relevantTechnologies = CORE_TECHNOLOGIES.filter(tech => {
    ...
    // Direct name matches
    if (queryLower.includes(techNameLower) || techNameLower.includes(queryLower)) {
      return true;
    }
    ...
    return tech.keyInnovations.some(innovation => 
  ...
  const relevantComponents = CORE_COMPONENTS.filter(comp => {
    ...
    // Direct name matches
    if (queryLower.includes(compNameLower) || compNameLower.includes(queryLower)) {
      return true;
    }
  ...
  for (const tech of relevantTechnologies) {
    ...
    evolution.successors.forEach(evo => {
      ...
      if (successorTech) {
        suggestedRelationships.push(`${tech.name} EVOLVED_TO ${successorTech.name}`);
      }
    });
  }
