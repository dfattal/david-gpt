...
import { DocumentChunk, PatentChunk, PatentSectionType, PatentMetadata, LeanPatentMetadata } from './types';
...
function parsePatentClaims(claims: string[]): {
  independentClaims: number[];
  dependentClaims: Array<{ claimNumber: number; dependsOn: number; text: string }>;
} {
  const independentClaims: number[] = [];
  const dependentClaims: Array<{ claimNumber: number; dependsOn: number; text: string }> = [];
  ...
  claims.forEach((claim, index) => {
    ...
    const dependencyPattern = /(?:claim|claims)\s+(\d+)(?:-(\d+))?|(?:any\s+of\s+claims?\s+)(\d+)(?:-(\d+))?/i;
 ...
 */
function parsePatentSections(fullText: string, extractedMetadata: PatentMetadata): PatentSections {
  ...
  if (sections.claims.length > 0) {
    const { independentClaims, dependentClaims } = parsePatentClaims(sections.claims);
  ...
  const sectionPatterns = {
    background: /(?:BACKGROUND|FIELD\s+OF\s+THE\s+INVENTION|RELATED\s+ART)(.*?)(?=(?:SUMMARY|BRIEF\s+DESCRIPTION|DETAILED\s+DESCRIPTION|CLAIMS)|$)/is,
    summary: /(?:SUMMARY|BRIEF\s+DESCRIPTION)(.*?)(?=(?:DETAILED\s+DESCRIPTION|CLAIMS)|$)/is,
    detailedDescription: /(?:DETAILED\s+DESCRIPTION|DESCRIPTION\s+OF\s+THE\s+INVENTION)(.*?)(?=CLAIMS|$)/is
  };
...
function createSpecialChunks(
  ...
  if (sections.abstract) {
    const enhancedAbstract = injectMetadataIntoContent(sections.abstract, {
      ...
      assignees: patentMetadata.assignee ? [patentMetadata.assignee] : undefined,
...
function createDependentClaimChunks(
  ...
  const claimGroups = new Map<number, Array<{ claimNumber: number; text: string }>>();
...
function chunkTextWithOverlap(
  ...
  for (let i = 0; i < sentences.length; i++) {
    ...
    if (currentTokens + sentenceTokens > config.maxTokens && currentChunk) {
      ...
      const overlapTokens = Math.floor(currentTokens * (config.overlapPercent / 100));
...
export function createPatentChunks(
  ...
  const specialChunks = createSpecialChunks(documentId, sections, config, extractedMetadata);
  const descriptionChunks = createDescriptionChunks(documentId, sections, config, specialChunks.length);
  ...
  // Combine all chunks
  const allChunks = [...specialChunks, ...descriptionChunks, ...dependentClaimChunks];
 ...
 */
function determinePatentStatus(grantedDate?: Date, filedDate?: Date): 'filed' | 'active' | 'expired' {
  const now = new Date();
  ...
  const expirationDate = new Date(baseDate.getTime() + 20 * 365 * 24 * 60 * 60 * 1000);
...
export function extractLeanPatentMetadata(
  ...
  const jurisdiction = extractedMetadata.patentNumber.match(/^([A-Z]{2})/)?.[1] || 'US';
  
  // Count claims
  const claimCount = extractedMetadata.claims?.length || 0;
  const { independentClaims } = parsePatentClaims(extractedMetadata.claims || []);
  ...
  return {
    ...
    expirationDate: extractedMetadata.grantedDate ? 
      new Date(extractedMetadata.grantedDate.getTime() + 20 * 365 * 24 * 60 * 60 * 1000) : // 20 years from grant
      undefined,
    expirationIsEstimate: true, // Mark as estimate since we're calculating it
    status: determinePatentStatus(extractedMetadata.grantedDate, extractedMetadata.filedDate),
    abstract: extractedMetadata.abstract || '',
    sourceUrl,
    authority,
    claimCount,
