...
export class LeiaArticleExtractor {
  ...
  private readonly displaySpecPatterns = {
    ...
    resolution: /\b(4K|8K|1080p|1440p|2160p|3840\s*[x×]\s*2160|1920\s*[x×]\s*1080)\b/gi
  };
  ...
  private readonly productCategoryPatterns = [
    { pattern: /\b(?:smart\s*)?TV(?:s)?|\btelevision(?:s)?\b/gi, category: 'TV' },
    { pattern: /\b(?:smart)?phone(?:s)?\b|\bmobile\s+device(?:s)?\b/gi, category: 'Smartphone' },
    ...
    { pattern: /\bgaming\s+(?:console|device|handheld)/gi, category: 'Gaming Console' },
    { pattern: /\bsmart\s+glass(?:es)?\b/gi, category: 'Smart Glasses' },
    { pattern: /\bheadset(?:s)?\b|\bVR\s+headset/gi, category: 'VR/AR Headset' },
   ...
   */
  public extractMetadata(title: string, content: string, url: string): Partial<PressArticleMetadata> {
  ...
  private extractOEM(content: string): string {
    for (const pattern of this.oemPatterns) {
      ...
      if (matches && matches.length > 0) {
        ...
        const oemCounts = matches.reduce((acc: Record<string, number>, match) => {
  ...
  private extractDisplayType(content: string): 'OLED' | 'LCD' | 'MicroLED' | 'Other' | undefined {
      ...
      if (matches) {
        matches.forEach(match => {
          const cleanFeature = match.trim()
            ...
            .replace(/\bimmersive\s+(?:gaming|experience|viewing)\b/gi, 'Immersive Experience')
            ...
            .replace(/\bLeia\s+(?:3D|technology|display|screen)\b/gi, 'Leia Technology');
  ...
  private extractPriceRange(content: string): string | undefined {
    ...
    if (priceDescMatches && priceDescMatches.length > 0) {
      return priceDescMatches[0].toLowerCase()
    ...
    for (const pattern of this.marketRegionPatterns) {
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          } else if (/europe|eu|uk|united\s+kingdom|germany|france|italy|spain/i.test(region)) {
            region = 'Europe';
          } else if (/asia|china|japan|south\s+korea|india|southeast\s+asia/i.test(region)) {
            region = 'Asia';
