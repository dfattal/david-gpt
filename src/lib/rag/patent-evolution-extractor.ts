...
export async function createTechnologyTimeline(
  ...
  try {
    ...
    const relevantPatents = documents.filter(
      doc =>
        doc.title.toLowerCase().includes(technologyName.toLowerCase()) ||
        (doc.patent_no &&
          LEIA_TECHNOLOGY_EVOLUTION.some(
            evo =>
    ...
    const timeline = relevantPatents
      .map(patent => {
        ...
        const evolution = LEIA_TECHNOLOGY_EVOLUTION.find(
          evo =>
            evo.fromPatent === patent.patent_no ||
            evo.toPatent === patent.patent_no
        );
        ...
        if (evolution) {
          if (evolution.fromPatent === patent.patent_no) {
            role =
              evolution.evolutionType === 'replacement'
                ? 'superseded'
                : 'enhanced';
 ...
 */
export async function getPatentEvolutionContext(patentNo: string): Promise<{
  predecessors: PatentEvolution[];
  successors: PatentEvolution[];
  relatedTechnologies: string[];
}> {
  const predecessors = LEIA_TECHNOLOGY_EVOLUTION.filter(
    evo => evo.toPatent === patentNo
  );
  const successors = LEIA_TECHNOLOGY_EVOLUTION.filter(
    evo => evo.fromPatent === patentNo
  );
...
export async function enhancePatentWithEvolutionContext(
  ...
  return {
    ...
    evolutionContext: {
      ...
      timelinePosition:
        evolutionContext.predecessors.length > 0
          ? 'evolution'
          : evolutionContext.successors.length > 0
            ? 'foundation'
            : 'standalone',
    },
  };
