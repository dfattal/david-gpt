...
export class AdvancedRerankingEngine {
  ...
  async rerank(
    ...
    console.log(
      `🔄 Starting advanced re-ranking: ${results.length} candidates → ${targetCount} results`
    );
    ...
    console.log(
      `📊 Over-retrieval: Using ${candidateCount} candidates for ${targetCount} final results`
    );
    ...
    switch (strategy) {
      case 'hybrid':
        rerankedResults = await this.hybridRerank(
          query,
          candidates,
          queryIntent
        );
      ...
      case 'semantic_similarity':
        rerankedResults = await this.semanticSimilarityRerank(
          query,
          candidates
        );
    ...
    const finalResults = await this.applyFinalFiltering(
      rerankedResults,
      targetCount
    );
    ...
    console.log(
      `   Strategy: ${strategy}, Final count: ${finalResults.length}`
    );
    console.log(
      `   Avg relevance: ${metrics.averageRelevance.toFixed(3)}, Diversity: ${metrics.diversityScore.toFixed(3)}`
    );
  ...
  private async hybridRerank(
    ...
    const [cohereResults, mmrResults, semanticResults] =
    ...
    const cohereScores = this.createScoreMap(
      cohereResults.status === 'fulfilled' ? cohereResults.value : candidates
    );
    const mmrScores = this.createScoreMap(
      mmrResults.status === 'fulfilled' ? mmrResults.value : candidates
    );
    const semanticScores = this.createScoreMap(
  ...
  private createScoreMap(results: SearchResult[]): Map<string, number> {
    ...
    results.forEach((result, index) => {
      const id = this.getResultId(result);
      const score =
        result.rerankScore || result.score || 1 - index / results.length;
  ...
  private async applyFinalFiltering(
    ...
    const filtered = results.filter(
      result =>
        (result.rerankScore || result.score || 0) >=
  ...
  private calculateRerankMetrics(
    ...
    const relevanceScores = finalResults.map(
      r => r.rerankScore || r.score || 0
    );
    ...
    const topKPrecision =
      topKScores.length > 0
        ? topKScores.reduce((sum, score) => sum + (score > 0.5 ? 1 : 0), 0) /
          topKScores.length
        : 0;
...
export async function advancedRerank(
  ...
  return await advancedRerankingEngine.rerank(
    query,
    results,
    queryIntent,
    targetCount
