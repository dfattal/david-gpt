...
export class HybridSearchEngine {
  ...
  async search(query: SearchQuery): Promise<HybridSearchResult> {
    ...
    try {
      console.log(
        `🔍 Starting hybrid search with relationship enhancement: "${query.query}"`
      );
      ...
      try {
        ...
        const relationshipResult =
          await relationshipSearchEngine.searchWithRelationships({
            query: query.query,
        ...
        if (relationshipResult.relationships.length > 0) {
          ...
          if (relatedEntities.length > 0) {
            ...
            console.log(
              `🚀 Enhanced query via relationships: "${enhancedQuery}"`
            );
            console.log(
              `🔗 Found ${relationshipResult.relationships.length} relationships`
            );
          }
        }
      } catch (relationshipError) {
        console.warn(
          '⚠️ Relationship search failed, falling back to regular hybrid search:',
          relationshipError
        );
      ...
      const typeAdjustedResults = this.applyDocumentTypeScoring(
        query.query,
        combinedResults
      );
      ...
      if (this.config.rerank && typeAdjustedResults.length > 0) {
        const { advancedRerank, QueryIntent } = await import(
          './advanced-reranking'
        );
        ...
        console.log(
          `   Avg relevance: ${rerankResult.metrics.averageRelevance.toFixed(3)}`
        );
        console.log(
          `   Diversity: ${rerankResult.metrics.diversityScore.toFixed(3)}`
        );
      }
      ...
      const limitedResults = finalResults.slice(
        0,
        query.limit || this.config.finalLimit
      );
      ...
      return {
        ...
        relationshipContext,
        enhancedQuery:
          enhancedQuery !== query.query ? enhancedQuery : undefined,
  ...
  private async semanticSearch(query: SearchQuery): Promise<SearchResult[]> {
    try {
      ...
      // Execute query
      const { data: chunks, error } = await supabaseQuery.limit(
      ...
      const results: SearchResult[] = chunks
        .map(chunk => {
          ...
          return {
            ...
            docType: chunk.documents?.document_types?.name || 'unknown',
        ...
        .filter(
          result => result.score >= (query.threshold || this.config.threshold)
        )
  ...
  private async keywordSearch(query: SearchQuery): Promise<SearchResult[]> {
    try {
      ...
      if (query.filters?.documentIds && query.filters.documentIds.length > 0) {
        ...
        const documentIdsParam = query.filters.documentIds
          .map(id => `'${id}'`)
          .join(',');
        const sqlQuery = `
          SELECT 
        ...
        if (error) {
          ...
          console.log(
            `🔍 Combined query succeeded: ${chunks?.length || 0} results`
          );
          ...
          const results: SearchResult[] =
            chunks?.map((chunk, index) => {
              ...
              return {
                ...
                title: chunk.documents?.title || 'Untitled',
                docType: chunk.documents?.document_types?.name || 'unknown',
                pageRange: this.formatPageRange(
                  chunk.page_start,
                  chunk.page_end
                ),
                sectionTitle: chunk.section_title,
          ...
          console.log(
            `🔍 Processed ${results.length} keyword results with combined filtering`
          );
          return results;
        }
      } else {
      ...
      // Execute query
      console.log(
        `🔍 About to execute Supabase query with maxResults: ${this.config.maxResults}`
      );
      ...
      if (query.filters?.documentIds && query.filters.documentIds.length > 0) {
        ...
        console.log(
          `🔍 Document filter test: ${testData?.length || 0} results`,
          testError || ''
        );
        ...
        const { data: textOnlyData, error: textOnlyError } =
          await textOnlyQuery;
        console.log(
          `🔍 Text-only search test: ${textOnlyData?.length || 0} results`,
          textOnlyError || ''
        );
        ...
        const { data: combinedData, error: combinedError } =
          await combinedQuery;
        console.log(
          `🔍 Combined search test: ${combinedData?.length || 0} results`,
          combinedError || ''
        );
      }
      ...
      console.log(
        `🔍 Keyword search raw results: ${chunks?.length || 0} chunks`
      );
      ...
      const results: SearchResult[] = chunks.map((chunk: any, index: any) => {
        ...
        return {
          ...
          content: chunk.content,
          title: chunk.documents?.title || 'Untitled',
          docType: chunk.documents?.document_types?.name,
          pageRange: this.formatPageRange(chunk.page_start, chunk.page_end),
          sectionTitle: chunk.section_title,
          metadata: chunk.documents as any,
        } as SearchResult;
   ...
   */
  private applyDocumentTypeScoring(
    query: string,
    results: SearchResult[]
  ): SearchResult[] {
    ...
    return results
      .map(result => {
        const docTypeBoost = this.calculateDocumentTypeBoost(
          queryIntent,
          result.docType,
          result.metadata
        );
        ...
        if (totalBoost !== 1.0) {
          console.log(
            `📊 Document type scoring: ${result.title.substring(0, 50)}... - ${result.docType} - Boost: ${totalBoost.toFixed(2)} (${result.score.toFixed(3)} → ${adjustedScore.toFixed(3)})`
          );
        }
  ...
  private classifyQueryIntent(
    query: string
  ): 'product' | 'technical' | 'academic' | 'general' {
    ...
    const productScore = productIndicators.filter(indicator =>
      lowerQuery.includes(indicator)
    ).length;
    const technicalScore = technicalIndicators.filter(indicator =>
      lowerQuery.includes(indicator)
    ).length;
    const academicScore = academicIndicators.filter(indicator =>
      lowerQuery.includes(indicator)
    ).length;
   ...
   */
  private calculateDocumentTypeBoost(
    intent: string,
    docType: string,
    metadata: any
  ): number {
   ...
   */
  private calculateProductNameBoost(
    query: string,
    result: SearchResult
  ): number {
    const lowerQuery = query.toLowerCase();
    ...
    const productPatterns = [
      // Samsung products
      {
        pattern: /samsung\s+odyssey\s+3d/i,
        boost: 3.0,
      ...
      {
        pattern: /lightfield\s+display/i,
        boost: 1.8,
    ...
    if (maxBoost > 1.0) {
      ...
      const titleMatch = productPatterns.some(
        ({ pattern }) =>
    ...
    if (maxBoost > 1.0) {
      console.log(
        `🎯 Product name boost: ${result.title.substring(0, 50)}... - Matched "${matchedProduct}" - Boost: ${maxBoost.toFixed(2)}`
      );
    }
  ...
  private applyFilters(query: any, filters?: SearchFilters): any {
    ...
    if (filters.documentTypes && filters.documentTypes.length > 0) {
      query = query.in('documents.document_types.name', filters.documentTypes);
    }
    ...
    if (filters.patents === true) {
      query = query.eq('documents.document_types.name', 'patent');
    }

    // Papers-specific filter
    if (filters.papers === true) {
      query = query.in('documents.document_types.name', ['paper', 'pdf']);
    }
...
export async function searchWithinDocument(
  ...
  try {
    ...
    return (chunks || []).map((chunk, index) => ({
      ...
      title: chunk.documents?.title || 'Untitled',
      docType: chunk.documents?.document_types?.name || 'unknown',
      pageRange: chunk.page_start ? `p. ${chunk.padStart}` : undefined,
      sectionTitle: chunk.section_title,
      metadata: chunk.documents as any,
    }));
  } catch (error) {
