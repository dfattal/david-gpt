...
export class EntityResolver {
  ...
  async resolveEntity(candidate: EntityCandidate): Promise<ResolutionResult> {
    ...
    try {
      ...
      if (bestMatch.similarityScore >= this.EXACT_MATCH_THRESHOLD) {
        return await this.mergeWithExistingEntity(
          bestMatch.existingEntity,
          candidate
        );
      } else if (bestMatch.similarityScore >= this.FUZZY_MATCH_THRESHOLD) {
        return await this.mergeWithFuzzyMatch(
          bestMatch.existingEntity,
          candidate,
  ...
  private normalizeEntityName(name: string): string {
    return name
      ...
      .replace(/\s+/g, ' ')
      .replace(
        /\b(?:inc|corp|ltd|llc|co|company|incorporated|corporation|limited)\b/g,
        ''
      )
  ...
  private async mergeWithExistingEntity(
    ...
    const { data: entity, error } = await supabaseAdmin
      ...
      .update({
        mention_count:
          existingEntity.mention_count + (candidate.mentionCount || 1),
   ...
   */
  private async mergeDuplicateEntities(
    keepEntity: Entity,
    mergeEntity: Entity
  ): Promise<void> {
    try {
      ...
      await supabaseAdmin
        ...
        .update({
          ...
          authority_score: Math.max(
            keepEntity.authority_score,
            mergeEntity.authority_score
          ),
          updated_at: new Date().toISOString(),
        })
        .eq('id', keepEntity.id);
      ...
      if (mergeEntity.name !== keepEntity.name) {
        await supabaseAdmin.from('aliases').insert({
      ...
      await supabaseAdmin.from('entities').delete().eq('id', mergeEntity.id);

      console.log(
        `✅ Merged entity "${mergeEntity.name}" into "${keepEntity.name}"`
      );
    } catch (error) {
...
export async function cleanupEntityDatabase(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `✅ Removed ${lowQualityEntities?.length || 0} low-quality entities`
    );
    ...
    console.log(
      `✅ Entity database cleanup completed. Merged ${totalMerged} duplicates.`
    );
  } catch (error) {
