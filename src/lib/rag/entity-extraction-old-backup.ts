 ...
 */
function detectEntityDocumentSection(
  content: string,
  entityText: string,
  docType: string
): DocumentSection {
...
function calculateEntityAuthorityScore(
  ...
  // Get base score
  const baseScores = isStructured
    ? BASE_AUTHORITY_SCORES.structured
    : BASE_AUTHORITY_SCORES.content;
  const baseScore = baseScores[entityKind] || 0.5;

  // Apply section modifier (only for content extraction)
  const sectionModifier = isStructured
    ? 1.0
    : SECTION_AUTHORITY_MODIFIERS[section];
...
function isValidEntity(name: string, entityType?: EntityKind): boolean {
  ...
  if (
    trimmed.length < MIN_ENTITY_LENGTH ||
    trimmed.length > MAX_ENTITY_LENGTH
  ) {
    return false;
  }
  ...
  for (const pattern of JUNK_PATTERNS) {
    if (pattern.test(trimmed)) {
      console.log(
        `❌ Rejected junk pattern: "${trimmed}" (matched: ${pattern})`
      );
      return false;
    }
  }
  ...
  if (meaningfulWords.length < MIN_MEANINGFUL_WORDS) {
    console.log(
      `❌ Rejected insufficient meaningful words: "${trimmed}" (only ${meaningfulWords.length})`
    );
    return false;
  }
  ...
  if (entityType && VALID_ENTITY_INDICATORS[entityType]) {
    ...
    const hasIndicator = indicators.some((pattern: any) =>
      pattern.test(trimmed)
    );

    // For technical domains, require some domain-specific indicators
    if (
      ['technology', 'component'].includes(entityType) &&
      !hasIndicator &&
      words.length < 2
    ) {
      console.log(
        `❌ Rejected ${entityType} without domain indicators: "${trimmed}"`
      );
      return false;
    }
  }
  ...
  console.log(
    `✅ Valid entity: "${trimmed}" (type: ${entityType || 'unknown'})`
  );
  return true;
}
...
export class EntityExtractor {
   ...
   */
  private extractMetadataRelationships(
    ...
    if (metadata.docType === 'patent') {
      const documentTitle =
        metadata.title || metadata.patentNo || 'Patent Document';
      ...
      if (
        ...
        typeof metadata.original_assignee === 'string'
      ) {
        const assigneeRelationships = METADATA_RELATIONSHIPS.assignee_of(
          [metadata.original_assignee],
          documentTitle
  ...
  private shouldKeepEntity(
    ...
    switch (entityKind) {
      case 'person':
        ...
        if (
          section === DocumentSection.CITATIONS ||
          section === DocumentSection.REFERENCES
        ) {
      ...
      for (const pattern of patterns) {
        ...
        while ((match = pattern.exec(content)) !== null) {
          ...
          if (
            srcEntity &&
            dstEntity &&
            isValidEntity(srcEntity) &&
            isValidEntity(dstEntity) &&
            srcEntity !== dstEntity
          ) {
            // Determine entity types based on relation context
            const { srcType, dstType } = this.inferEntityTypes(
              relationType,
              srcEntity,
              dstEntity
            );
  ...
  private inferEntityTypes(
    ...
    const relationMappings: Record<
      string,
      { srcType: EntityKind; dstType: EntityKind }
    > = {
    ...
    const defaultTypes = relationMappings[relation] || {
      srcType: 'technology',
      dstType: 'technology',
    };
    ...
    const srcType =
      this.inferEntityTypeFromName(srcEntity) || defaultTypes.srcType;
    const dstType =
      this.inferEntityTypeFromName(dstEntity) || defaultTypes.dstType;
  ...
  private inferEntityTypeFromName(entityName: string): EntityKind | null {
    ...
    // Product patterns
    if (
      /\b(iphone|ipad|quest|vision pro|unity|sdk|runtime|platform)\b/.test(name)
    ) {
      return 'product';
    }
    ...
    // Technology patterns
    if (
      /\b(display|technology|algorithm|method|technique|system)\b/.test(name)
    ) {
      return 'technology';
    }
  ...
  async extractFromDocument(
    ...
    if (!chunks || !Array.isArray(chunks)) {
      console.error(
        '❌ Chunks parameter is undefined or not an array:',
        chunks
      );
      return { entities, aliases, relationships };
    }
    const fullContent = chunks.map(chunk => chunk.content).join('\n\n');
    const contentEntities = this.extractFromContent(
      fullContent,
      metadata.docType
    );
    ...
    if (metadata.docType === 'patent') {
      const patentEntities = await this.extractFromPatentContent(
        fullContent,
        metadata
      );
      ...
      const pressEntities = this.extractFromPressArticleContent(
        fullContent,
        metadata
      );
    ...
    console.log(
      `✅ Extracted ${entities.length} entities, ${aliases.length} aliases, ${relationships.length} relationships`
    );
  ...
  private extractFromMetadata(metadata: DocumentMetadata & any): {
    ...
    console.log(
      `🔍 Extracting from metadata for ${metadata.docType} document: ${metadata.title}`
    );
    ...
    if (metadata.docType === 'patent') {
      ...
      if (
        ...
        if (isValidEntity(originalAssignee, 'organization')) {
          ...
          const alreadyIncluded = assignees?.some(
            (assignee: string) =>
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          if (
            ...
            isValidEntity(trimmedMatch, 'technology')
          ) {
            found.add(normalized);

            // Detect section and calculate mention count
            const section = detectEntityDocumentSection(
              content,
              trimmedMatch,
              docType
            );
            ...
            const authorityScore = calculateEntityAuthorityScore(
              'technology',
              false,
              section,
              mentionCount
            ...
            const shouldKeep = this.shouldKeepEntity(
              'technology',
              authorityScore,
              section,
              mentionCount
            ...
            if (shouldKeep) {
              ...
              console.log(
                `🔧 Technology entity: "${trimmedMatch}" | Section: ${section} | Authority: ${authorityScore} | Mentions: ${mentionCount}`
              );
            } else {
              console.log(
                `❌ Filtered technology entity: "${trimmedMatch}" | Section: ${section} | Authority: ${authorityScore} (too low)`
              );
            }
          }
        });
    ...
    PRODUCT_PATTERNS.forEach(pattern => {
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          if (
            !found.has(normalized) &&
            isValidEntity(trimmedMatch, 'product')
          ) {
            found.add(normalized);
            ...
            if (shouldKeep) {
              ...
              console.log(
                `📱 Product entity: "${trimmedMatch}" | Section: ${section} | Authority: ${authorityScore} | Mentions: ${mentionCount}`
              );
            } else {
              console.log(
                `❌ Filtered product entity: "${trimmedMatch}" | Section: ${section} | Authority: ${authorityScore} (too low)`
              );
            }
          }
        });
    ...
    personPatterns.forEach(pattern => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (
          name.length > 5 &&
          !found.has(normalized) &&
          !/\b(?:the|and|or|for|with|from|that|this|was|were|been|have|has|will|would|could|should)\b/i.test(
            name
          )
        ) {
          found.add(normalized);
          ...
          if (shouldKeep) {
            ...
            console.log(
              `👤 Person entity: "${name}" | Section: ${section} | Authority: ${authorityScore} | Mentions: ${mentionCount}`
            );
          } else {
            console.log(
              `❌ Filtered person entity: "${name}" | Section: ${section} | Authority: ${authorityScore} (too low)`
            );
          }
        }
      }
    ...
    COMPONENT_PATTERNS.forEach(pattern => {
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          if (
            !found.has(normalized) &&
            isValidEntity(trimmedMatch, 'component')
          ) {
            found.add(normalized);
            ...
            if (shouldKeep) {
              ...
              console.log(
                `🔧 Component entity: "${trimmedMatch}" | Section: ${section} | Authority: ${authorityScore} | Mentions: ${mentionCount}`
              );
            } else {
              console.log(
                `❌ Filtered component entity: "${trimmedMatch}" | Section: ${section} | Authority: ${authorityScore} (too low)`
              );
            }
          }
        });
   ...
   */
  private async extractFromPatentContent(
    content: string,
    metadata: DocumentMetadata
  ): Promise<{
    entities: Partial<Entity>[];
    ...
    if (metadata.patentNo) {
      try {
        const { extractPatentEvolutionRelationships } = await import(
          './patent-evolution-extractor'
        );
        const evolutionRelationships =
          await extractPatentEvolutionRelationships(
            metadata.patentNo,
            content,
            metadata
          );
        ...
        console.log(
          `🔄 Extracted ${evolutionRelationships.length} evolution relationships for patent ${metadata.patentNo}`
        );
      } catch (error) {
        console.warn(
          'Failed to extract patent evolution relationships:',
          error
        );
      }
    }
    ...
    inventionPatterns.forEach(pattern => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (
          invention.length > 10 &&
          invention.length < 80 &&
          isValidEntity(invention, 'product')
        ) {
          entities.push({
            name: invention,
            kind: 'product' as EntityKind,
   ...
   */
  private extractFromPaperContent(
    content: string,
    metadata: DocumentMetadata
  ): {
    entities: Partial<Entity>[];
   ...
   */
  private extractFromPressArticleContent(
    content: string,
    metadata: DocumentMetadata
  ): {
    entities: Partial<Entity>[];
    ...
    const formattedRelationships = pressResult.relationships.map(
      (rel: any) => ({
        srcName: rel.srcName,
        srcType: rel.srcType,
    ...
    console.log(
      `📰 Press article extraction: ${pressResult.entities.length} entities, ${formattedRelationships.length} relationships`
    );
  ...
  private deduplicateEntities(entities: Partial<Entity>[]): Partial<Entity>[] {
    ...
    entities.forEach(entity => {
      ...
      if (!matchedKey && entity.kind === 'organization') {
        for (const [key, existing] of deduped.entries()) {
          if (
            existing.kind === 'organization' &&
            this.isOrganizationVariant(entity.name, existing.name!)
          ) {
            matchedKey = key;
            bestMatch = existing;
            console.log(
              `🔗 Organization variant matched: "${entity.name}" → "${existing.name}"`
            );
            break;
          }
        }
      ...
      if (
        !matchedKey &&
        (entity.kind === 'technology' || entity.kind === 'component')
      ) {
        for (const [key, existing] of deduped.entries()) {
          if (
            ...
            bestMatch = existing;
            console.log(
              `🔗 Semantic match: "${entity.name}" → "${existing.name}"`
            );
            break;
          }
        }
      ...
      if (bestMatch) {
        ...
        bestMatch.mentionCount =
          (bestMatch.mentionCount || 0) + (entity.mentionCount || 0);
        bestMatch.authorityScore = Math.max(
          bestMatch.authorityScore || 0,
    ...
    console.log(
      `🔀 Deduplication: ${entities.length} → ${result.length} entities`
    );
    return result;
  }
  ...
  private isOrganizationVariant(name1: string, name2: string): boolean {
    const normalize = (name: string) =>
      name
        .toLowerCase()
        .replace(
          /\b(inc|corp|corporation|ltd|limited|llc|co|company|gmbh|technologies|tech|systems|solutions|industries|electronics|semiconductors)\b\.?/g,
          ''
        )
        .replace(/[.,\s]+/g, ' ')
        .trim();
   ...
   */
  private isSemanticallyEquivalent(
    name1: string,
    name2: string,
    ...
    for (const [canonical, variants] of Object.entries(variations)) {
      if (
        (norm1 === canonical && variants.includes(norm2)) ||
  ...
  async saveEntities(
    ...
    console.log(
      `💾 Saving ${entities.length} entities to database with canonical consolidation...`
    );

    try {
      // Import consolidators
      const { entityConsolidator } = await import('./entity-consolidator');
      const { consolidateEntityName, getCanonicalRelationships } = await import(
        './canonical-entities'
      );
      ...
      console.log(
        `✅ Entities saved: ${canonicalConsolidationCount} canonically consolidated, ${reusedCount} reused, ${newCount} new`
      );
      ...
      const updatedRelationships = relationships.map(rel => {
        ...
        if (consolidatedEntities.has(rel.srcName)) {
          ...
          console.log(
            `🔄 Updated relationship source: ${rel.srcName} → ${updatedRel.srcName}`
          );
        }
        if (consolidatedEntities.has(rel.dstName)) {
          updatedRel.dstName = consolidatedEntities.get(rel.dstName);
          console.log(
            `🔄 Updated relationship destination: ${rel.dstName} → ${updatedRel.dstName}`
          );
        }
        return updatedRel;
      });
      ...
      for (const [
        originalName,
        canonicalName,
      ] of consolidatedEntities.entries()) {
      ...
      console.log(
        `🔗 Saved ${allRelationships.length} relationships (${canonicalRelationships.length} canonical) to database`
      );
    } catch (error) {
      ...
      for (const rel of relationships) {
        ...
        if (srcEntity && dstId) {
          ...
          if (error) {
            ...
            console.log(
              `✅ Saved relationship: ${rel.srcName} ${rel.relation} ${rel.dstName}`
            );
          }
        } else {
          console.warn(
            `⚠️ Missing entities for relationship: ${rel.srcName} → ${rel.dstName}`
          );
        }
      }
    } catch (error) {
...
export async function processDocumentEntities(
  ...
  try {
    ...
    // Extract entities
    const { entities, aliases, relationships } =
      await entityExtractor.extractFromDocument(
        documentId,
        documentMetadata,
    ...
    await entityExtractor.saveEntities(
      documentId,
      entities,
      aliases,
      relationships
    ...
    console.log(
      `✅ Entity extraction completed for document: ${document.title}`
    );
  } catch (error) {
