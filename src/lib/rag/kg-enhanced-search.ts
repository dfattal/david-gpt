...
export class KGEnhancedSearchEngine extends HybridSearchEngine {
  ...
  > {
    ...
    try {
      ...
      const finalResults = this.sortEnhancedResults(
        enhancedResults,
        authorityBoosts
      );
  ...
  async searchEntities(
    ...
    console.log(
      `🔍 Searching entities: "${entityName}" (type: ${entityType || 'any'})`
    );
    ...
    try {
      ...
      for (const event of events) {
        ...
        if (event.document_id) {
          const docResults = await this.getDocumentSearchResult(
            event.document_id
          );
  ...
  private async recognizeEntities(queryText: string): Promise<Entity[]> {
    ...
    const potentialEntities =
      queryText.match(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b/g) || [];
  ...
  private async expandQueryWithEntities(
    ...
    const expandedQueryText = [query.query, ...expansionTerms.slice(0, 5)].join(
      ' OR '
    );
   ...
   */
  private async getDocumentSearchResult(
    documentId: string
  ): Promise<SearchResult[]> {
    try {
