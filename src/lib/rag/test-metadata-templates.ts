...
import { generateMetadataFooter, injectMetadataIntoContent } from './metadata-templates.js';
...
function runTests() {
  ...
  const originalAbstract = "A multi-view display is switchable between single view and multi-view modes...";
  const enhancedAbstract = injectMetadataIntoContent(originalAbstract, testPatent);
