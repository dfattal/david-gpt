...
async function testTechnicalExtraction() {
  ...
  try {
    ...
    console.log(
      `📊 Technical term density: ${result.metadata.technicalTermDensity.toFixed(1)} per 1000 words`
    );
    console.log(
      `🎯 Average authority score: ${result.metadata.avgAuthorityScore.toFixed(2)}`
    );
    console.log(
      `❓ FAQ structure detected: ${result.metadata.faqStructureDetected ? '✅' : '❌'}\n`
    );
    ...
    for (const critical of criticalEntities) {
      ...
      if (found) {
        foundCritical++;
        console.log(
          `   ✅ ${critical}: "${found.name}" (authority: ${found.authorityScore.toFixed(2)}, mentions: ${found.mentionCount})`
        );
      } else {
    ...
    const entitiesByKind = result.entities.reduce(
      (acc, entity) => {
    ...
    for (const [kind, entities] of Object.entries(entitiesByKind)) {
      ...
      sorted.forEach((entity, i) => {
        console.log(
          `     ${i + 1}. "${entity.name}" (authority: ${entity.authorityScore.toFixed(2)}, mentions: ${entity.mentionCount})`
        );
      });
    }
    ...
    result.relationships.slice(0, 10).forEach((rel, i) => {
      console.log(
        `   ${i + 1}. "${rel.srcName}" --${rel.relation}--> "${rel.dstName}" (confidence: ${rel.confidence.toFixed(2)})`
      );
    });
    ...
    console.log('\n⚡ Performance Analysis:');
    console.log(
      `   Entities per second: ${Math.round(result.entities.length / (extractionTime / 1000))}`
    );
    console.log(
      `   Processing rate: ${Math.round(content.length / extractionTime)} chars/ms`
    );

    // Quality assessment
    console.log('\n🎯 Quality Assessment:');
    const highQualityEntities = result.entities.filter(
      e => e.authorityScore >= 0.7
    ).length;
    const qualityPercentage =
      (highQualityEntities / result.entities.length) * 100;
    console.log(
      `   High quality entities (≥0.7): ${highQualityEntities}/${result.entities.length} (${qualityPercentage.toFixed(1)}%)`
    );

    const technicalEntities = result.entities.filter(
      e => e.kind === 'technology' || e.kind === 'component'
    ).length;
    console.log(
      `   Technical entities: ${technicalEntities}/${result.entities.length} (${Math.round((technicalEntities / result.entities.length) * 100)}%)`
    );
    ...
    console.log(
      `🎯 Overall extraction score: ${(assessmentScore * 100).toFixed(1)}%`
    );
    ...
    if (assessmentScore >= 0.9) {
      ...
      console.log(
        '✅ Good! Technical extraction working well with minor improvements needed.'
      );
    } else if (assessmentScore >= 0.5) {
      console.log(
        '⚠️  Fair! Technical extraction needs significant improvements.'
      );
    } else {
