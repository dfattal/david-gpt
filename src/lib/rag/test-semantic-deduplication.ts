...
async function testSemanticDeduplication() {
  ...
  const existingEntities: Partial<Entity>[] = [
    { name: 'Nubia Pad 3D II', kind: 'product', mentionCount: 5, authorityScore: 0.8 },
    { name: 'Leia Inc.', kind: 'organization', mentionCount: 3, authorityScore: 0.9 },
    { name: 'Samsung Galaxy S24 Ultra', kind: 'product', mentionCount: 8, authorityScore: 0.95 },
    { name: 'David Chen', kind: 'person', mentionCount: 2, authorityScore: 0.7 },
    { name: 'Glasses-Free 3D Tablet', kind: 'product', mentionCount: 4, authorityScore: 0.75 }
  ];
  ...
  const testCases = [
    ...
    {
      entity: { name: 'announced the Nubia Pad 3D II', kind: 'product' as const },
    ...
    try {
      ...
      if (result.canonicalEntity) {
        ...
        if (result.canonicalEntity.mentionCount) {
          console.log(`   📈 Merged mention count: ${result.canonicalEntity.mentionCount}`);
        }
      }
      ...
      if (testCase.entity.name.includes('iPhone 15 Pro')) {
      ...
      } else if (testCase.entity.name.includes('Nubia Pad 3D II') || 
        ...
        expectedCorrect = result.similarity >= 0.85 ? result.action === 'merge' : result.action === 'create';
      }
  ...
  console.log('=' .repeat(80));
  console.log(`Correct actions: ${correctActions}/${totalTests} (${Math.round(correctActions/totalTests*100)}%)`);
...
async function testSimilarityAlgorithms() {
  ...
  const testPairs = [
    { entity1: 'Nubia Pad 3D II', entity2: 'announced the Nubia Pad 3D II', expected: 'high' },
    { entity1: 'Leia Inc.', entity2: 'Leia', expected: 'high' },
    { entity1: 'Samsung Galaxy S24 Ultra', entity2: 'Galaxy S24 Ultra', expected: 'high' },
    { entity1: 'Glasses-Free 3D Tablet', entity2: 'Free 3D Tablet', expected: 'high' },
    { entity1: 'iPhone 15 Pro', entity2: 'Samsung Galaxy S24 Ultra', expected: 'low' },
  ...
  for (const pair of testPairs) {
    ...
    const actualLevel = result.similarity >= 0.85 ? 'high' : result.similarity >= 0.3 ? 'medium' : 'low';
    const correct = actualLevel === pair.expected;
    
    console.log(`   Result: ${actualLevel} similarity ${correct ? '✅' : '❌'}`);
    console.log('');
  }
}
...
export { runSemanticDeduplicationTests, testSemanticDeduplication, testSimilarityAlgorithms };