...
export class PerformanceTestSuite {
  ...
  async runFullTestSuite(): Promise<TestSuiteResults> {
    console.log('🚀 Starting Performance Test Suite for Three-Tier RAG Architecture\n');
    ...
    for (let i = 0; i < TEST_QUERIES.length; i++) {
      ...
      console.log(`Expected: ${testQuery.expectedTier.toUpperCase()} tier, <${testQuery.expectedPerformance}ms`);
      ...
      console.log(`${status} Result: ${result.actualTier.toUpperCase()} tier, ${result.executionTime}ms, ${result.resultCount} results`);
      console.log(`${tierStatus} Tier Classification | ${perfStatus} Performance Target`);
   ...
   */
  private async testSingleQuery(testQuery: TestQuery): Promise<PerformanceResult> {
    const startTime = Date.now();

    try {
      // Step 1: Test query classification
      const classification = classifySearchQuery(testQuery.query);
      const tierClassificationCorrect = classification.tier === testQuery.expectedTier;
  ...
  private generateTestSummary(): TestSuiteResults {
    ...
    const correctClassifications = this.results.filter(r => r.tierClassificationCorrect).length;
    const performanceTargetsMet = this.results.filter(r => r.performanceMet).length;
  ...
  private calculateTierMetrics(tier: string) {
    ...
    return {
      ...
      avgTime: tierResults.length > 0
        ? tierResults.reduce((sum, r) => sum + r.executionTime, 0) / tierResults.length
        : 0,
      successRate: tierResults.length > 0
  ...
  printResults(results: TestSuiteResults): void {
    ...
    // Overall Statistics
    console.log('\n📊 Overall Performance:');
    console.log(`✅ Successful Queries: ${results.successfulQueries}/${results.totalQueries} (${((results.successfulQueries/results.totalQueries)*100).toFixed(1)}%)`);
    console.log(`🎯 Tier Classification Accuracy: ${results.tierClassificationAccuracy.toFixed(1)}%`);
    console.log(`⚡ Performance Targets Met: ${results.performanceTargetsMet}/${results.totalQueries} (${((results.performanceTargetsMet/results.totalQueries)*100).toFixed(1)}%)`);
    console.log(`⏱️  Average Execution Time: ${results.averageExecutionTime.toFixed(0)}ms`);
    ...
    console.log('\n📈 Performance Analysis:');
    const sqlQueries = results.results.filter(r => r.expectedTier === 'sql' && r.success);
    const fastSqlQueries = sqlQueries.filter(r => r.executionTime < 200);
    console.log(`⚡ SQL Tier Speed: ${fastSqlQueries.length}/${sqlQueries.length} queries <200ms (${((fastSqlQueries.length/sqlQueries.length)*100).toFixed(1)}%)`);

    const vectorQueries = results.results.filter(r => r.expectedTier === 'vector' && r.success);
    const reasonableVectorQueries = vectorQueries.filter(r => r.executionTime < 1000);
    console.log(`🎯 Vector Tier Speed: ${reasonableVectorQueries.length}/${vectorQueries.length} queries <1000ms (${((reasonableVectorQueries.length/vectorQueries.length)*100).toFixed(1)}%)`);
    ...
    const misclassified = results.results.filter(r => !r.tierClassificationCorrect);
    if (misclassified.length > 0) {
      console.log('\n🔄 Misclassified Queries:');
      misclassified.forEach(result => {
        console.log(`  "${result.query}": Expected ${result.expectedTier.toUpperCase()}, got ${result.actualTier.toUpperCase()}`);
      });
    }
...
export async function runPerformanceTests(): Promise<void> {
  ...
  try {
    ...
    console.log('\n🎯 Success Criteria Evaluation:');
    console.log(`✅ Search Success Rate: ${results.successfulQueries >= results.totalQueries * 0.95 ? 'PASS' : 'FAIL'} (${((results.successfulQueries/results.totalQueries)*100).toFixed(1)}% >= 95%)`);
    console.log(`✅ Tier Classification: ${results.tierClassificationAccuracy >= 90 ? 'PASS' : 'FAIL'} (${results.tierClassificationAccuracy.toFixed(1)}% >= 90%)`);
    console.log(`✅ Performance Targets: ${results.performanceTargetsMet >= results.totalQueries * 0.8 ? 'PASS' : 'FAIL'} (${((results.performanceTargetsMet/results.totalQueries)*100).toFixed(1)}% >= 80%)`);

    const sqlAvgTime = results.tierBreakdown.sql.avgTime;
    console.log(`✅ SQL Tier Speed: ${sqlAvgTime < 200 ? 'PASS' : 'FAIL'} (${sqlAvgTime.toFixed(0)}ms < 200ms)`);
