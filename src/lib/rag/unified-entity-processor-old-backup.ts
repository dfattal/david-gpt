...
const EXTRACTION_STRATEGIES: Record<string, EntityExtractionStrategy> = {
  ...
  comprehensive: {
    name: 'Comprehensive',
    description:
      'Main extraction using comprehensive patterns and section awareness',
    primary: true,
    qualityThresholds: {
      person: 0.4,
      organization: 0.3,
  ...
  press: {
    name: 'Press Article',
    description:
      'Specialized extraction for press articles with product relationships',
    primary: false,
    qualityThresholds: {
      person: 0.6,
      organization: 0.7,
...
export class UnifiedEntityProcessor {
  ...
  async extractEntities(
    ...
    if (result.relationships.length > 0) {
      ...
      result.relationships.slice(0, 3).forEach(rel => {
        console.log(
          `     ${rel.srcName} (${rel.srcType}) --${rel.relation}--> ${rel.dstName} (${rel.dstType}) [${rel.confidence}]`
        );
      });
    }

    // 3. Apply quality filtering and consolidation
    const filteredResult = await this.applyQualityFiltering(
      result,
      strategy,
      metadata,
      chunks
    ...
    console.log(
      `   Entities: ${filteredResult.entities.length}, Relationships: ${filteredResult.relationships.length}`
    );
  ...
  private analyzeExtractionContext(
    ...
    const domainSpecific = domainKeywords.some(keyword =>
  ...
  private selectExtractionStrategy(
    ...
    if (isTechnical) {
      console.log(
        `✅ Selected strategy: Technical Documentation (${isTechnical.reason})`
      );
    ...
    const isPress =
      context.documentType === 'press-article' ||
      this.isPressArticleContent(context);
    if (isPress) {
      console.log(
        `✅ Selected strategy: Press Article (documentType = ${context.documentType}, content detected = ${this.isPressArticleContent(context)})`
      );
    ...
    if (context.domainSpecific && !isPress && !isTechnical) {
      console.log(
        `✅ Selected strategy: Domain-Specific (domainSpecific = true, not press/technical)`
      );
    ...
    if (
      context.hasStructuredMetadata &&
      ['patent', 'paper', 'pdf'].includes(context.documentType)
    ) {
      console.log(
        `✅ Selected strategy: Comprehensive (hasStructuredMetadata = true, docType = ${context.documentType})`
      );
    ...
    if (context.contentLength > 10000) {
      console.log(
        `✅ Selected strategy: Comprehensive (contentLength = ${context.contentLength})`
      );
      return EXTRACTION_STRATEGIES.comprehensive;
    }
    ...
    if (context.documentType === 'note') {
      ...
      if (context.chunks && context.chunks.length > 0) {
        ...
        if (technicalDensity > 2) {
          // More than 2 technical terms per 100 words
  ...
  private async executeTechnical(
    ...
    const technicalResult =
      await technicalDocumentEntityExtractor.extractEntities(
        fullContent,
        metadata,
        chunks
      );
    ...
    const entities = technicalResult.entities.map((e: any) => ({
      name: e.name,
      kind: e.kind,
  ...
  private async executeComprehensive(
    ...
    return await entityExtractor.extractFromDocument(
      documentId,
      metadata,
      chunks
    );
  ...
  private async executeFallback(
    ...
    const filteredEntities = result.entities.filter(
      (entity: any) =>
        (entity.authorityScore || 0) > 0.5 && (entity.mentionCount || 0) > 1
    );
  ...
  private async applyQualityFiltering(
    ...
    const { assessEntitiesQuality } = await import(
      './entity-quality-framework'
    );
    ...
    const filteredProfiles = qualityProfiles.filter(profile => {
      ...
      const minThreshold =
        strategy.qualityThresholds[profile.entity.kind!] || 0.3;
    ...
    const filteredRelationships = result.relationships.filter(
      rel => (rel.confidence || 0) >= 0.4
    );

    console.log(
      `🔍 Quality filtering: ${result.entities.length} → ${filteredEntities.length} entities`
    );
    console.log(
      `   Quality distribution: ${filteredProfiles.map(p => p.qualityTier).join(', ')}`
    );
  ...
  async saveExtractedData(
    ...
    console.log(
      `💾 Saving ${extractionResult.entities.length} entities and ${extractionResult.relationships.length} relationships...`
    );
    ...
    console.log(
      `🔀 In-memory deduplication: ${extractionResult.entities.length} → ${deduplicatedEntities.length} entities`
    );

    // 2. Apply semantic deduplication against existing database entities
    const semanticallyFilteredEntities =
      await this.applySemanticDeduplication(deduplicatedEntities);
    console.log(
      `🧠 Semantic deduplication: ${deduplicatedEntities.length} → ${semanticallyFilteredEntities.length} entities`
    );
    ...
    const validRelationships = extractionResult.relationships.filter(rel => {
      if (!rel.srcName || !rel.dstName || !rel.relation) {
        console.warn(
          `⚠️  Invalid relationship: ${rel.srcName} --${rel.relation}--> ${rel.dstName}`
        );
        return false;
      }
      return true;
    ...
    console.log(
      `🔍 Relationship validation: ${extractionResult.relationships.length} -> ${validRelationships.length} valid relationships`
    );
    ...
    for (const rel of validRelationships) {
      try {
        ...
        if (!srcEntity || !dstEntity) {
          console.warn(
            `⚠️  Skipping relationship - missing entities: ${rel.srcName} (${srcEntity ? '✓' : '✗'}) -> ${rel.dstName} (${dstEntity ? '✓' : '✗'})`
          );
          continue;
        }

        // Create entity-to-entity relationship
        const { error: relError } = await supabaseAdmin.from('edges').insert({
          ...
          evidence_text:
            `${rel.srcName} (${rel.srcType}) -> ${rel.dstName} (${rel.dstType}): ${rel.evidenceText}`.substring(
              0,
              500
            ),
        ...
        if (relError && relError.code !== '23505') {
          // Ignore duplicates
          console.error(
            `❌ Error saving relationship ${rel.srcName} -> ${rel.dstName}:`,
            relError
          );
          ...
          console.log(
            `🔄 Relationship already exists: ${rel.srcName} -> ${rel.relation} -> ${rel.dstName}`
          );
        } else {
          console.log(
            `✅ Saved entity relationship: ${rel.srcName} -> ${rel.relation} -> ${rel.dstName}`
          );
        }
      } catch (err) {
        console.error(
          `❌ Exception saving relationship ${rel.srcName} -> ${rel.dstName}:`,
          err
        );
      }
    }
  ...
  private async applySemanticDeduplication(
    ...
    console.log(
      `🧠 Starting semantic deduplication for ${entities.length} entities...`
    );
    ...
    for (const kind of entityKinds) {
      ...
      if (existingEntities) {
        ...
        console.log(
          `🔍 Found ${existingEntities.length} existing ${kind} entities for comparison`
        );
      }
    }
    ...
    for (const newEntity of entities) {
      if (!newEntity.name || !newEntity.kind) {
        rejectedEntities.push({
          entity: newEntity,
          reason: 'Missing name or kind',
        });
        continue;
      }

      const existingOfSameKind =
        existingEntitiesByKind.get(newEntity.kind) || [];

      // INTRA-BATCH DEDUPLICATION: Also check against entities already processed in this batch
      const alreadyProcessedOfSameKind = filteredEntities.filter(
        e => e.kind === newEntity.kind
      );
      const allEntitiesForComparison = [
        ...existingOfSameKind,
        ...alreadyProcessedOfSameKind,
      ];

      // Check for semantic similarity with existing entities AND entities already processed in this batch
      const deduplicationResult =
        await semanticEntityDeduplicator.checkEntityDuplication(
          newEntity,
          allEntitiesForComparison
        );

      console.log(
        `🔍 "${newEntity.name}" (${newEntity.kind}): ${deduplicationResult.action} - ${deduplicationResult.explanation}`
      );
      ...
      switch (deduplicationResult.action) {
        ...
        case 'merge':
          ...
          if (deduplicationResult.canonicalEntity) {
            await this.updateExistingEntityStats(
              deduplicationResult.canonicalEntity,
              newEntity
            );
            mergedEntities.push({
              entity: newEntity,
        ...
        case 'reject':
          ...
          rejectedEntities.push({
            entity: newEntity,
            reason: deduplicationResult.explanation,
          });
          break;
      }
    }
   ...
   */
  private async updateExistingEntityStats(
    canonicalEntity: Partial<Entity>,
    newEntity: Entity
  ): Promise<void> {
    ...
    try {
      const { error } = await supabaseAdmin
        ...
        .update({
          mention_count:
            (canonicalEntity.mentionCount || 0) + (newEntity.mentionCount || 1),
          authority_score: Math.max(
            canonicalEntity.authorityScore || 0,
            newEntity.authorityScore || 0
          ),
      ...
      if (error) {
        console.error(
          `❌ Error updating existing entity "${canonicalEntity.name}":`,
          error
        );
      } else {
        console.log(
          `🔄 Updated stats for existing entity: "${canonicalEntity.name}"`
        );
      }
    } catch (error) {
      console.error(
        `❌ Exception updating existing entity "${canonicalEntity.name}":`,
        error
      );
    }
  }
    ...
    try {
      ...
      if (existingEntity) {
        ...
        const { error: updateError } = await supabaseAdmin
          ...
          .update({
            mention_count:
              (existingEntity.mention_count || 0) + (entity.mentionCount || 0),
            authority_score: Math.max(
              existingEntity.authority_score || 0,
              entity.authorityScore || 0
            ),
            description:
        ...
        if (updateError) {
          console.error(
            `❌ Error updating entity ${entity.name}:`,
            updateError
          );
        } else {
          console.log(
            `🔄 Updated existing entity: ${entity.name} (${entity.kind})`
          );
        }
      } else {
        // Insert new entity
        ...
        if (insertError && insertError.code !== '23505') {
          ...
          console.error(
            `❌ Error inserting entity ${entity.name}:`,
            insertError
          );
        } else if (insertError?.code === '23505') {
          console.log(
            `🔄 Entity already exists: ${entity.name} (${entity.kind})`
          );
        } else {
          console.log(
            `✅ Inserted new entity: ${entity.name} (${entity.kind})`
          );
        }
      }
    } catch (error) {
  ...
  private isPressArticleContent(context: ExtractionContext): boolean {
    ...
    console.log(
      `🔍 Press article detection: ${matchCount} indicators found (threshold: ${threshold}) -> ${isPress ? 'YES' : 'NO'}`
    );
 ...
 */
async function createAuthorDocumentRelationships(
  documentId: string,
  document: any
): Promise<void> {
  console.log(
    `🔗 Creating author-document relationships for: ${document.title}`
  );
  ...
  try {
    ...
    if (document.authors_affiliations) {
      try {
        ...
        if (typeof document.authors_affiliations === 'string') {
          console.log(
            `📝 Parsing JSON string: ${document.authors_affiliations.substring(0, 100)}...`
          );
        ...
        const authorsArray = Array.isArray(authorsData)
          ? authorsData
          : [authorsData];
        ...
        for (const authorEntry of authorsArray) {
          ...
          if (typeof authorEntry === 'string') {
            ...
            if (authorEntry.name && typeof authorEntry.name === 'string') {
              ...
              authorEntry.author &&
              typeof authorEntry.author === 'string'
            ) {
    ...
    if (document.inventors) {
      ...
      try {
        ...
        // Ensure we have an array
        const inventorsArray = Array.isArray(inventorsData)
          ? inventorsData
          : [inventorsData];
        ...
        for (const inventorEntry of inventorsArray) {
          ...
          if (typeof inventorEntry === 'string') {
            ...
            if (inventorEntry.name && typeof inventorEntry.name === 'string') {
              ...
              inventorEntry.inventor &&
              typeof inventorEntry.inventor === 'string'
            ) {
    ...
    if (authors.length === 0) {
      console.log(
        `ℹ️  No authors/inventors found for document: ${document.title}`
      );
      return;
    }

    console.log(
      `👥 Found ${authors.length} authors/inventors: ${authors.join(', ')}`
    );
    ...
    for (const authorName of authors) {
      ...
      try {
        ...
        console.log(
          `👤 Author entity ready: ${authorEntity.name} (${authorEntity.id})`
        );
        ...
        console.error(
          `❌ Failed to create relationship for author ${authorName}:`,
          err
        );
      }
    }
    ...
    console.log(
      `✅ Successfully created ${successfulRelationships}/${authors.length} author-document relationships`
    );
  } catch (error) {
 ...
 */
async function findOrCreateAuthorEntity(
  authorName: string
 ...
 */
async function createAuthorDocumentEdge(
  authorEntityId: string,
  documentId: string,
  authorName: string
): Promise<void> {
  console.log(
    `🔍 Creating edge: ${authorName} (${authorEntityId}) -> document (${documentId})`
  );
  ...
  if (existingEdge) {
    console.log(
      `🔄 Author-document relationship already exists: ${authorName} (${existingEdge.id})`
    );
    return;
  }
  ...
  console.log(
    `✅ Created author-document relationship: ${authorName} -> document (edge: ${newEdge?.id})`
  );
}
...
export async function processDocumentEntities(
  ...
  console.log(
    `🚀 Starting unified entity processing for document: ${documentId}`
  );
  ...
  try {
    ...
    const { data: document, error: docError } = await supabaseAdmin
      .from('documents')
      .select(
        'id, title, doc_type, patent_no, doi, arxiv_id, published_date, authors_affiliations, inventors'
      )
      .eq('id', documentId)
      .single();
    ...
    const result = await unifiedEntityProcessor.extractEntities(
      documentId,
      metadata,
      documentChunks
    );
    ...
    if (result.entities.length === 0) {
      console.log(
        '✅ No entities extracted from this document, but author relationships created'
      );
      return;
    }
    ...
    console.log(
      `✅ Unified entity processing completed for document: ${document.title}`
    );
    ...
    console.log(
      `   Avg Authority: ${result.metadata.avgAuthorityScore.toFixed(2)}`
    );
  } catch (error) {
