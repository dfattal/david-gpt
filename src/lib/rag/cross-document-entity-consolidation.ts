...
export class CrossDocumentEntityConsolidator {
  ...
  async consolidateEntities(): Promise<ConsolidationResult> {
    ...
    for (const [entityType, entities] of entitiesByType.entries()) {
      ...
      const typeResult = await this.consolidateEntitiesByType(entities, entityType);
      ...
      console.log(`   ✅ ${entityType}: ${typeResult.entitiesMerged} merges, ${typeResult.aliasesCreated} aliases`);
    }
    
    // Calculate overall quality metrics
    const qualityMetrics = this.calculateConsolidationQuality(allMergeDecisions, totalProcessed, totalMerged);
    
    const executionTime = Date.now() - startTime;
    
    console.log(`✅ Cross-document consolidation completed in ${executionTime}ms`);
    console.log(`   Processed: ${totalProcessed} entities`);
    console.log(`   Merged: ${totalMerged} entities (${(totalMerged/totalProcessed*100).toFixed(1)}%)`);
  ...
  private async consolidateEntitiesByType(
    ...
    console.log(`   📊 Created ${clusters.length} clusters from ${entities.length} entities`);
    ...
    return {
      ...
      qualityMetrics: this.calculateConsolidationQuality(mergeDecisions, entitiesProcessed, entitiesMerged),
      mergeDecisions
    };
  }
   ...
   */
  private async createEntityClusters(entities: Entity[]): Promise<EntityCluster[]> {
    ...
    for (const entity of entities) {
      ...
      if (cluster.entities.length > 1) {
        cluster.averageSimilarity = this.calculateClusterSimilarity(cluster.entities);
      }
      
      // Determine primary entity (highest authority)
      cluster.primaryEntity = cluster.entities.reduce((best, current) =>
        (current.authorityScore || 0) > (best.authorityScore || 0) ? current : best
      );
  ...
  private async findSimilarEntities(
    ...
    const candidates = candidateEntities.slice(0, this.config.maxConsolidationCandidates);
  ...
  private calculateEntitySimilarity(entity1: Entity, entity2: Entity): number {
    ...
    if (!this.config.enableCrossTypeConsolidation && entity1.kind !== entity2.kind) {
      return 0;
    }
    ...
    if (entity1.description && entity2.description) {
      const descSimilarity = this.calculateTextSimilarity(entity1.description, entity2.description);
      bonusScore += descSimilarity * 0.1;
    }
    
    // Similar authority scores (indicates similar extraction confidence)
    const authDiff = Math.abs((entity1.authorityScore || 0) - (entity2.authorityScore || 0));
    if (authDiff < 0.2) {
      bonusScore += 0.05;
    }
  ...
  private calculateNameSimilarity(name1: string, name2: string): number {
    ...
    const prefixBonus = Math.min(0.1, prefixMatch / Math.max(normalized1.length, normalized2.length));
    const suffixBonus = Math.min(0.1, suffixMatch / Math.max(normalized1.length, normalized2.length));
   ...
   */
  private async mergeEntityCluster(
    ...
    const entitiesToMerge = cluster.entities.filter(e => e.id !== primaryEntity.id);
    
    if (entitiesToMerge.length === 0) {
      return { aliasesCreated: 0, relationshipsUpdated: 0, mergeDecision: null };
    }
    
    console.log(`   🔗 Merging ${entitiesToMerge.length} entities into "${primaryEntity.name}"`);
    ...
    if (updateError) {
      console.error(`Failed to update primary entity ${primaryEntity.id}:`, updateError);
      return { aliasesCreated: 0, relationshipsUpdated: 0, mergeDecision: null };
    }
    
    // Create aliases for merged entities
    for (const entityToMerge of entitiesToMerge) {
      if (this.config.enableAliasDetection && entityToMerge.name !== primaryEntity.name) {
        const { error: aliasError } = await supabaseAdmin
          .from('entity_aliases')
          .insert({
            entity_id: primaryEntity.id,
            alias: entityToMerge.name,
            confidence: this.calculateEntitySimilarity(primaryEntity, entityToMerge),
        ...
        if (!aliasError) {
          ...
          console.warn(`Failed to create alias "${entityToMerge.name}" for "${primaryEntity.name}":`, aliasError);
        }
      }
    }
  ...
  private mergeAuthority(
    ...
    switch (this.config.authorityMergeStrategy) {
      ...
      case 'weighted_sum':
        ...
        return (authority1 * mentions1 + authority2 * mentions2) / totalMentions;
  ...
  private calculateConsolidationQuality(
    ...
    const avgSimilarityScore = mergeDecisions.reduce(
      (sum, decision) => sum + decision.similarityScore, 0
    ) / mergeDecisions.length;
    
    const confidenceScore = mergeDecisions.reduce(
      (sum, decision) => sum + decision.confidence, 0
    ) / mergeDecisions.length;
    
    const consolidationRatio = totalProcessed > 0 ? totalMerged / totalProcessed : 0;
    
    // Calculate authority improvement (simplified)
    const authorityImprovement = mergeDecisions.reduce(
      (sum, decision) => sum + Math.max(0, decision.mergedAuthority - 0.5), 0
    ) / mergeDecisions.length;
...
export const crossDocumentEntityConsolidator = new CrossDocumentEntityConsolidator();
...
export async function consolidateEntitiesByType(
  ...
  if (error || !entities) {
    throw new Error(`Failed to fetch ${entityType} entities: ${error?.message}`);
  }
  
  return await consolidator['consolidateEntitiesByType'](entities as Entity[], entityType);
}