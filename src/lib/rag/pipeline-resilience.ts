...
export class PipelineResilienceManager {
  ...
  async executeWithResilience<T>(
    ...
    const finalRetryConfig: RetryConfig = {
      ...
      nonRetryableErrors:
        retryConfig?.nonRetryableErrors || NON_RETRYABLE_ERRORS,
    };
    ...
    if (this.isCircuitBreakerOpen(operationName)) {
      ...
      if (fallback && this.config.enableFallbacks) {
        console.log(
          `🔄 Circuit breaker open for ${operationName}, using fallback`
        );
        try {
    ...
    while (attempts < finalRetryConfig.maxAttempts) {
      ...
      try {
        console.log(
          `🔄 Executing ${operationName} (attempt ${attempts}/${finalRetryConfig.maxAttempts})`
        );
  ...
  private isCircuitBreakerOpen(operationName: string): boolean {
    ...
    if (state.state === 'open') {
      ...
      if (
        ...
        console.log(
          `🔄 Circuit breaker for ${operationName} moving to half-open state`
        );
      }
  ...
  private recordSuccess(operationName: string): void {
    ...
    if (state) {
      ...
      if (state.state === 'half_open') {
        ...
        console.log(
          `✅ Circuit breaker for ${operationName} closed after successful operation`
        );
      }
    }
  }
  ...
  private async performHealthChecks(): Promise<void> {
    const services = ['supabase', 'openai_embeddings', 'cohere_rerank'];
  ...
  private calculateErrorRate(service: string): number {
    const recentErrors = this.errorLog.filter(
      error =>
        error.component === service && Date.now() - error.timestamp < 300000 // Last 5 minutes
    );
  ...
  private logError(error: ProcessingError): void {
    ...
    console.error(
      `🚨 Pipeline Error [${error.component}/${error.operation}]:`,
      {
...
export function getSystemHealth(): {
  ...
  const healthy =
    Array.from(healthStatus.values()).every(status => status.healthy) &&
