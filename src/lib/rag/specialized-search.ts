...
export async function searchAuthorAdvanced(
  ...
  try {
    ...
    const authorEntities = await kgEnhancedSearchEngine.searchEntities(
      authorName,
      'person',
      3
    );
    ...
    if (!author) {
      ...
      return {
        author: null,
        documents: await kgEnhancedSearchEngine.searchByAuthor(
          authorName,
          options.limit || 20
        ),
      };
...
export async function searchByCoAuthors(
  ...
  try {
    ...
    const allDocuments = new Map<
      string,
      { doc: SearchResult; authorCount: number }
    >();

    for (const author of authors) {
      const authorDocs = await kgEnhancedSearchEngine.searchByAuthor(
        author.name,
        100
      );
...
export async function searchTechnologyAdvanced(
  ...
  try {
    ...
    const techEntities = await kgEnhancedSearchEngine.searchEntities(
      technologyName,
      'algorithm',
      3
    );
    ...
    if (!technology) {
      return {
        technology: null,
        documents: await kgEnhancedSearchEngine.searchByTechnology(
          technologyName,
          options.limit || 20
        ),
      };
    ...
    if (options.includeImplementations) {
      results.implementations = await findTechnologyImplementations(
    ...
    if (options.includeRelatedTech) {
      results.relatedTechnologies = await findRelatedTechnologies(
        technology.entity.id,
        5
      );
    }
    ...
    if (options.includeContext) {
      ...
      results.entities = await Promise.all(
        entityNames.map(name =>
          kgEnhancedSearchEngine.searchEntities(name!, undefined, 1)
        )
...
export async function searchEventsByPeriod(
  ...
  console.log(
    `📆 Period events search: ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`
  );
  ...
  try {
    ...
    if (inventorNames.size > 0) {
      results.inventors = await Promise.all(
        Array.from(inventorNames)
          .slice(0, 10)
          .map(name => kgEnhancedSearchEngine.searchEntities(name, 'person', 1))
      ).then(results => results.map(r => r[0]).filter(Boolean));
    }
  ...
  try {
    ...
    if (relationships) {
      for (const rel of relationships) {
        if (
          rel.entities &&
          rel.entities.entity_kinds &&
          rel.entities.entity_kinds.name === 'org'
        ) {
          affiliations.push({
            entity: rel.entities as any,
            aliases: [],
 ...
 */
async function findTechnologyImplementations(
  technologyId: string
): Promise<EntitySearchResult[]> {
  try {
    const { data: relationships } = await supabaseAdmin
    ...
    if (relationships) {
      for (const rel of relationships) {
        if (
          rel.entities &&
          rel.entities.entity_kinds &&
          rel.entities.entity_kinds.name === 'product'
        ) {
 ...
 * Find related technologies
 */
async function findRelatedTechnologies(
  technologyId: string,
  limit: number
): Promise<EntitySearchResult[]> {
  try {
    ...
    if (relationships) {
      for (const rel of relationships) {
        if (
          rel.entities &&
          rel.entities.entity_kinds &&
          rel.entities.entity_kinds.name === 'algorithm'
        ) {
          relatedTech.push({
            entity: rel.entities as any,
            aliases: [],
...
function filterDocuments(
  ...
  if (options.timeRange) {
    filtered = filtered.filter(doc => {
      ...
      if (options.timeRange!.start && date < options.timeRange!.start)
        return false;
