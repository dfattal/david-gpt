...
export class SearchAnalyticsService {
  ...
  async logSearchEvent(event: SearchAnalyticsEvent): Promise<void> {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`📊 Search Analytics: ${event.tier.toUpperCase()} | ${event.executionTime}ms | ${event.resultCount} results | ${event.query}`);
    }
  }
   ...
   */
  getTierDistribution(timeFrame?: { start: Date; end: Date }): TierDistributionStats {
    ...
    return {
      sql: {
        ...
        avgTime: sqlEvents.length > 0
          ? sqlEvents.reduce((sum, e) => sum + e.executionTime, 0) / sqlEvents.length
          : 0
      },
      vector: {
        count: vectorEvents.length,
        percentage: total > 0 ? (vectorEvents.length / total) * 100 : 0,
        avgTime: vectorEvents.length > 0
          ? vectorEvents.reduce((sum, e) => sum + e.executionTime, 0) / vectorEvents.length
          : 0
      },
      content: {
        count: contentEvents.length,
        percentage: total > 0 ? (contentEvents.length / total) * 100 : 0,
        avgTime: contentEvents.length > 0
          ? contentEvents.reduce((sum, e) => sum + e.executionTime, 0) / contentEvents.length
          : 0
      },
      total
    };
   ...
   */
  getPerformanceMetrics(timeFrame?: { start: Date; end: Date }): PerformanceMetrics {
    let events = this.events;
    ...
    return {
      ...
      sqlTierPerformance: {
        averageTime: sqlSuccessful.length > 0
          ? sqlSuccessful.reduce((sum, e) => sum + e.executionTime, 0) / sqlSuccessful.length
          : 0,
        under200ms: sqlSuccessful.filter(e => e.executionTime < 200).length,
        total: sqlEvents.length,
        successRate: sqlEvents.length > 0 ? (sqlSuccessful.length / sqlEvents.length) * 100 : 0
      },

      vectorTierPerformance: {
        averageTime: vectorSuccessful.length > 0
          ? vectorSuccessful.reduce((sum, e) => sum + e.executionTime, 0) / vectorSuccessful.length
          : 0,
        under1000ms: vectorSuccessful.filter(e => e.executionTime < 1000).length,
        total: vectorEvents.length,
        successRate: vectorEvents.length > 0 ? (vectorSuccessful.length / vectorEvents.length) * 100 : 0
      },

      contentTierPerformance: {
        averageTime: contentSuccessful.length > 0
          ? contentSuccessful.reduce((sum, e) => sum + e.executionTime, 0) / contentSuccessful.length
          : 0,
        total: contentEvents.length,
        successRate: contentEvents.length > 0 ? (contentSuccessful.length / contentEvents.length) * 100 : 0
      },

      overallSuccessRate: events.length > 0 ? (successfulEvents.length / events.length) * 100 : 0,
   ...
   */
  private calculateClassificationAccuracy(events: SearchAnalyticsEvent[]): number {
  ...
  printAnalyticsReport(): void {
    ...
    console.log(`SQL Tier: ${distribution.sql.count} (${distribution.sql.percentage.toFixed(1)}%) - Avg: ${distribution.sql.avgTime.toFixed(0)}ms`);
    console.log(`Vector Tier: ${distribution.vector.count} (${distribution.vector.percentage.toFixed(1)}%) - Avg: ${distribution.vector.avgTime.toFixed(0)}ms`);
    console.log(`Content Tier: ${distribution.content.count} (${distribution.content.percentage.toFixed(1)}%) - Avg: ${distribution.content.avgTime.toFixed(0)}ms`);

    // Performance Metrics
    console.log('\n⚡ Performance Metrics:');
    console.log(`Overall Average: ${metrics.averageExecutionTime.toFixed(0)}ms`);
    console.log(`Overall Success Rate: ${metrics.overallSuccessRate.toFixed(1)}%`);

    console.log(`\nSQL Tier Performance:`);
    console.log(`  Average Time: ${metrics.sqlTierPerformance.averageTime.toFixed(0)}ms`);
    console.log(`  Under 200ms: ${metrics.sqlTierPerformance.under200ms}/${metrics.sqlTierPerformance.total} (${((metrics.sqlTierPerformance.under200ms/metrics.sqlTierPerformance.total)*100).toFixed(1)}%)`);
    console.log(`  Success Rate: ${metrics.sqlTierPerformance.successRate.toFixed(1)}%`);

    console.log(`\nVector Tier Performance:`);
    console.log(`  Average Time: ${metrics.vectorTierPerformance.averageTime.toFixed(0)}ms`);
    console.log(`  Under 1000ms: ${metrics.vectorTierPerformance.under1000ms}/${metrics.vectorTierPerformance.total} (${((metrics.vectorTierPerformance.under1000ms/metrics.vectorTierPerformance.total)*100).toFixed(1)}%)`);
    console.log(`  Success Rate: ${metrics.vectorTierPerformance.successRate.toFixed(1)}%`);

    console.log(`\nContent Tier Performance:`);
    console.log(`  Average Time: ${metrics.contentTierPerformance.averageTime.toFixed(0)}ms`);
    console.log(`  Success Rate: ${metrics.contentTierPerformance.successRate.toFixed(1)}%`);
    ...
    if (patterns.length > 0) {
      ...
      patterns.forEach((pattern, index) => {
        console.log(`${index + 1}. [${pattern.tier.toUpperCase()}] "${pattern.query}" - ${pattern.frequency}x, ${pattern.avgTime.toFixed(0)}ms avg`);
      });
    }
...
export async function logSearch(
  ...
  resultCount: number,
  success: boolean,
  classification: { confidence: number; explanation: string; matchType: string },
  options?: {
    fallbackUsed?: boolean;
    executionStrategy?: string;
    userId?: string;
