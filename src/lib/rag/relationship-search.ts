...
export class RelationshipSearchEngine {
  ...
  async searchWithRelationships(params: {
    ...
    console.log(
      `📊 Found ${queryEntities.length} entities in query:`,
    ...
    if (includeRelatedEntities && queryEntities.length > 0) {
      const expansion = await this.expandEntitiesViaRelationships(
        queryEntities,
        maxHops
      );
      ...
      console.log(
        `🔍 Expanded to ${expandedEntities.length} total entities via relationships`
      );
    }
    ...
    const enrichedDocuments = await this.addRelationshipContext(
      documents,
      relationships
    );
  ...
  private async expandEntitiesViaRelationships(
    ...
    console.log(
      `🔗 Found ${relationships.length} relationships and ${expandedEntities.length} related entities`
    );
    ...
    return documents.map(doc => {
      ...
      return {
        ...
        relationshipContext: relevantRels.map(rel => ({
          ...
          relatedEntity:
            rel.source === doc.title ? rel.destination : rel.source,
  ...
  try {
    ...
    if (results.relationships.length > 0) {
      ...
      results.relationships.forEach((rel, i) => {
        console.log(
          `  ${i + 1}. ${rel.source} --[${rel.relationship}]--> ${rel.destination} (${rel.weight})`
        );
        if (rel.evidence) {
    ...
    if (results.documents.length > 0) {
      ...
      results.documents.slice(0, 3).forEach((doc, i) => {
        console.log(
          `  ${i + 1}. "${doc.title}" (relevance: ${doc.relevanceScore.toFixed(2)})`
        );
        if (doc.relationshipContext && doc.relationshipContext.length > 0) {
          doc.relationshipContext.forEach((ctx: any) => {
            console.log(
              `     → Related via "${ctx.relationship}" to "${ctx.relatedEntity}"`
            );
          });
        }
      });
