...
import { entityQualityAssessor, type EntityQualityProfile } from './entity-quality-framework';
...
const problematicEntities: Array<{entity: Partial<Entity>, description: string}> = [
...
function testProblematicEntities() {
  console.log('🔍 Testing Problematic Entities (should be caught by validation)\n');
  ...
  problematicEntities.forEach(({ entity, description }) => {
    ...
    totalIssues++;
    const hasValidationIssues = profile.metrics.validationScore < 0.5 || 
    ...
    if (hasValidationIssues) {
      ...
      console.log(`   Validation Score: ${profile.metrics.validationScore.toFixed(3)}`);
    ...
    if (profile.issues.length > 0) {
      console.log(`   Issues: ${profile.issues.map(i => i.description).join('; ')}`);
    }
    console.log('');
  });
  
  console.log(`📊 Results: Caught ${caughtIssues}/${totalIssues} problematic entities (${Math.round(caughtIssues/totalIssues*100)}%)\n`);
...
function testGoodEntities() {
  ...
  goodEntities.forEach(({ entity, description }) => {
    ...
    totalEntities++;
    const passedValidation = profile.metrics.validationScore >= 0.5 && 
                            profile.qualityTier !== 'rejected' &&
  ...
  console.log(`📊 Results: ${passedEntities}/${totalEntities} good entities passed validation (${Math.round(passedEntities/totalEntities*100)}%)\n`);
...
function testCrossTypeDuplicates() {
  ...
  const profiles = entityQualityAssessor.assessEntities(duplicateEntities, mockContext);
  
  profiles.forEach(profile => {
    const hasDuplicateIssues = profile.issues.some(issue => 
      issue.type === 'duplicate' && issue.description.includes('Cross-type')
    );
    
    console.log(`${hasDuplicateIssues ? '🔍' : '📋'} "${profile.entity.name}" (${profile.entity.kind})`);
    console.log(`   Quality Tier: ${profile.qualityTier}`);
    console.log(`   Overall Score: ${profile.metrics.overallScore.toFixed(3)}`);
    
    if (hasDuplicateIssues) {
      const duplicateIssue = profile.issues.find(issue => 
        issue.type === 'duplicate' && issue.description.includes('Cross-type')
      );
...
async function runEntityQualityTests() {
  ...
  try {
    ...
    console.log('=' .repeat(60));
    console.log(`Problematic entities caught: ${problematicResults.caughtIssues}/${problematicResults.totalIssues} (${Math.round(problematicResults.caughtIssues/problematicResults.totalIssues*100)}%)`);
    console.log(`Good entities passed: ${goodResults.passedEntities}/${goodResults.totalEntities} (${Math.round(goodResults.passedEntities/goodResults.totalEntities*100)}%)`);
    
    const overallSuccess = (problematicResults.caughtIssues / problematicResults.totalIssues + 
                           goodResults.passedEntities / goodResults.totalEntities) / 2;
    
    console.log(`Overall validation accuracy: ${Math.round(overallSuccess * 100)}%`);
...
export { runEntityQualityTests, testProblematicEntities, testGoodEntities, testCrossTypeDuplicates };