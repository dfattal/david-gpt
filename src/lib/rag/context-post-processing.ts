...
export class ContextPostProcessor {
  ...
  async processContext(
    ...
    console.log(
      `🔄 Processing context: ${searchResults.length} results → max ${maxTokens} tokens`
    );
    ...
    const initialTokens = segments.reduce(
    ...
    console.log(
      `📊 Initial analysis: ${segments.length} segments, ${initialTokens} tokens`
    );

    // Step 2: Redundancy detection and removal
    const stepStart2 = Date.now();
    const deduplicatedSegments = await this.removeRedundancy(segments);
    const tokensAfterDedup = deduplicatedSegments.reduce(
      (sum, seg) => sum + seg.tokenCount,
      0
    );
    ...
    console.log(
      `🔄 After deduplication: ${deduplicatedSegments.length} segments, ${tokensAfterDedup} tokens`
    );
    ...
    const tokensAfterSelection = selectedSegments.reduce(
      (sum, seg) => sum + seg.tokenCount,
      0
    );
    ...
    if (tokensAfterSelection > maxTokens && this.config.enableSummarization) {
      ...
      const tokensAfterCompression = finalSegments.reduce(
        (sum, seg) => sum + seg.tokenCount,
        0
      );
      ...
      console.log(
        `📝 After compression: ${finalSegments.length} segments, ${tokensAfterCompression} tokens`
      );
    }

    // Step 5: Coherence optimization and assembly
    const stepStart5 = Date.now();
    const { content, citations } = await this.assembleContext(
      finalSegments,
      queryIntent
    );
    ...
    const compressionRatio =
      initialTokens > 0 ? finalTokens / initialTokens : 1.0;
    ...
    console.log(
      `   ${initialTokens} → ${finalTokens} tokens (${Math.round(compressionRatio * 100)}% of original)`
    );
    console.log(
      `   Quality score: ${qualityMetrics.overallQuality.toFixed(3)}`
    );
    ...
    return {
      ...
      compressionRatio,
      sourceCount: new Set(finalSegments.map(s => s.sourceResult.documentId))
        .size,
      citations,
      qualityMetrics,
      processingSteps,
  ...
  private async removeRedundancy(
    ...
    for (const segment of segments) {
      ...
      if (isRedundant && groupIndex >= 0) {
        ...
        const mostImportant = redundancyGroups[groupIndex].reduce(
          (best, current) =>
  ...
  private sortForComparison(segments: ContentSegment[]): ContentSegment[] {
    ...
    const maxPerType = Math.max(
      1,
      Math.floor(segments.length / byDocType.size)
    );
  ...
  private balanceSourceTypes(
    ...
    while (
      ...
      typeArrays.some((arr, i) => typeIndices[i] < arr.length)
    ) {
  ...
  private async assembleContext(
    ...
    for (let i = 0; i < orderedSegments.length; i++) {
      ...
      citations.push({
        ...
        extractedContent:
          segment.summary || segment.content.substring(0, 150) + '...',
  ...
  private orderByCausality(segments: ContentSegment[]): ContentSegment[] {
    ...
    return segments.sort((a, b) => {
      const aCausal = causalWords.some(word =>
        a.content.toLowerCase().includes(word)
      );
      const aEffect = effectWords.some(word =>
        a.content.toLowerCase().includes(word)
      );
      const bCausal = causalWords.some(word =>
        b.content.toLowerCase().includes(word)
      );
      const bEffect = effectWords.some(word =>
        b.content.toLowerCase().includes(word)
      );
  ...
  private async calculateQualityMetrics(
    ...
    const relevanceScore =
      segments.length > 0
        ? segments.reduce((sum, seg) => sum + seg.relevanceScore, 0) /
          segments.length
        : 0;
  ...
  private calculateCompletenessScore(
    ...
    const wordCoverage =
      queryWords.length > 0 ? coveredWords.length / queryWords.length : 1;

    // Check source diversity
    const uniqueSources = new Set(segments.map(s => s.sourceResult.documentId))
      .size;
  ...
  private calculateCitationRelevance(
    ...
    const contentWords = new Set(
      content.split(/\s+/).filter(w => w.length > 3)
    );
    const responseWords = new Set(
      fullContent.split(/\s+/).filter(w => w.length > 3)
    );
    const overlap = new Set(
      [...contentWords].filter(word => responseWords.has(word))
    );
    const overlapScore =
      contentWords.size > 0 ? overlap.size / contentWords.size : 0;
    ...
    const typeRelevance = this.getDocumentTypeRelevance(
      segment.sourceResult.docType
    );
  ...
  private calculateCitationRelevanceMetrics(
    ...
    const relevantCitations = citations.filter(
      c => c.relevanceScore >= relevanceThreshold
    );
    ...
    const confidenceScore =
      totalCitations > 0 ? relevantCount / totalCitations : 1.0;
...
export async function processSearchContext(
  ...
  return await contextPostProcessor.processContext(
    query,
    searchResults,
    queryIntent,
    maxTokens
