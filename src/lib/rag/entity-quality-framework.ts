...
import type { Entity, EntityKind, DocumentMetadata, DocumentChunk } from './types';
...
export interface QualityIssue {
  type: 'pattern_violation' | 'context_mismatch' | 'low_authority' | 'duplicate' | 'incomplete' | 'noise';
...
const DEFAULT_QUALITY_CONFIG: QualityConfig = {
  ...
  weights: {
    ...
    context: 0.20,      // Contextual relevance and placement
...
export class EntityQualityAssessor {
  ...
  assessEntity(
    ...
    if (!entity.name || !entity.kind) {
      return {
        ...
        issues: [{ type: 'incomplete', severity: 'critical', description: 'Entity missing required name or kind' }],
    ...
    const recommendations = this.generateRecommendations(entity, metrics, issues);
  ...
  private calculateAuthorityScore(
    entity: Partial<Entity>,
    context: { extractionSource: string; sectionContext?: string; documentMetadata: DocumentMetadata }
  ): number {
    ...
    if (context.sectionContext) {
      const sectionMultiplier = this.getSectionAuthorityMultiplier(context.sectionContext);
  ...
  private calculateStructuralScore(
    ...
    switch (docType) {
      case 'patent':
        ...
        return entity.kind === 'person' || entity.kind === 'organization' ? 1.0 : 0.8;
      case 'paper':
        // Academic papers have structured author/venue data
        return entity.kind === 'person' || entity.kind === 'organization' ? 0.9 : 0.6;
      default:
        return 0.5;
    }
  }
  ...
  private getSectionAuthorityMultiplier(sectionContext: string): number {
    ...
    if (section.includes('reference') || section.includes('citation') || section.includes('bibliography')) {
      return 0.6;
    }
   ...
   */
  private getValidationRules(kind: EntityKind): Array<{ test: (name: string) => boolean; weight: number }> {
    ...
    rules.push(
      ...
      { test: (name) => !/^(of|and|or|the|in|at|to|for|with|from|by|on|bringing|featuring|using|including)\s/i.test(name), weight: 0.9 },
      { test: (name) => !/\s(of|and|or|the|in|at|to|for|with|from|by|on)$/i.test(name), weight: 0.9 },
      
      // Stop words - reject entities that are just common words
      { test: (name) => !/^(professional|premium|standard|basic|advanced|new|old|current|recent|latest|next|first|last|main|primary|secondary|additional|special|general|international|global|worldwide|annual|monthly|weekly|daily)$/i.test(name), weight: 0.95 },
      
      // Event/venue filtering
      { test: (name) => !/^(CES|MWC|IFA|Computex|SIGGRAPH|GDC|E3|PAX|Gamescom|NAB|CEATEC)\s*\d{4}$/i.test(name), weight: 0.7 },
      { test: (name) => !/^(Hall|Booth|Room|Floor|Level|Stage|Area|Zone|Pavilion|Section)\s*\d+/i.test(name), weight: 0.7 },
      
      // Technical specification filtering - reject raw specs as entities
      { test: (name) => !/^\d+[\-\"′″]?\s*(inch|inches|mm|cm|Hz|fps|GB|TB|MB|mAh|W|V|A)(?:es)?$/i.test(name), weight: 0.8 },
      { test: (name) => !/^(A\s*)?\d+(\.\d+)?\s*(inch|GB|TB|mAh|Hz|fps|W|V|A)/i.test(name), weight: 0.8 },
      
      // Price and currency filtering
      { test: (name) => !/^\$\d+|\d+\s*(USD|EUR|GBP|dollars?|euros?|pounds?)$/i.test(name), weight: 0.9 },
    ...
    switch (kind) {
      case 'person':
        rules.push(
          { test: (name) => /^[A-Z][a-z]+(\s+[A-Z]\.?\s*)*[A-Z][a-z]+$/.test(name), weight: 0.8 }, // Proper name format
          { test: (name) => name.split(' ').length >= 2 && name.split(' ').length <= 4, weight: 0.6 }, // Reasonable name length
          { test: (name) => !/\b(the|and|or|of|in|at|to|for)\b/i.test(name), weight: 0.4 } // No common words
        );
        break;
      ...
      case 'organization':
        rules.push(
          ...
          { test: (name) => !/^(Samsung|Apple|LG|Sony|TCL|Hisense|Microsoft|Google|Amazon|Meta|Intel|AMD|NVIDIA|Qualcomm)\s+(Galaxy|iPhone|iPad|MacBook|Pixel|Surface|Xbox|PlayStation|Kindle|Quest|GeForce|Radeon|Snapdragon)/i.test(name), weight: 0.9 }
        );
        break;
      ...
      case 'component':
        rules.push(
          { test: (name) => name.length >= 3 && name.length <= 50, weight: 0.5 }, // Reasonable length
          { test: (name) => !/^(new|old|recent|current|existing|additional)$/i.test(name), weight: 0.6 }, // Not just adjectives
      ...
      case 'product':
        rules.push(
          ...
          { test: (name) => !/^(new|old|recent|current|existing|additional|professional|premium|standard|basic|advanced)$/i.test(name), weight: 0.7 }, // Not just adjectives
          { test: (name) => /[A-Za-z]/.test(name), weight: 0.8 }, // Contains letters
          // Prevent company names from being classified as products
          { test: (name) => !/^(Samsung|Apple|LG|Sony|TCL|Hisense|Microsoft|Google|Amazon|Meta|Intel|AMD|NVIDIA|Qualcomm|Panasonic|Sharp|Philips|Vizio|BOE|CSOT|AUO|Innolux|Nintendo|ASUS|MSI|OnePlus|Xiaomi|Huawei|OPPO|Vivo|Motorola)$/i.test(name), weight: 0.95 }
        );
        break;
  ...
  private identifyQualityIssues(
    ...
    if (metrics.authorityScore < 0.3) {
      issues.push({
        ...
        suggestion: 'Verify entity through additional sources or structured metadata'
      });
    }
    ...
    if (metrics.validationScore < 0.4) {
      issues.push({
        ...
        severity: 'minor',
        description: 'Entity name does not match expected patterns for its kind',
    ...
    if (metrics.contextScore < 0.3) {
      issues.push({
        ...
        severity: 'minor',
        description: 'Entity may not be contextually relevant to document content',
    ...
    if (metrics.mentionScore < 0.2) {
      issues.push({
        ...
        suggestion: 'Consider filtering entities with very low mention frequencies'
      });
    }
  ...
  private generateRecommendations(
    ...
    if (metrics.overallScore < 0.5) {
      recommendations.push('Consider additional validation before including in knowledge graph');
    }
    
    if (issues.some(issue => issue.type === 'low_authority')) {
      recommendations.push('Cross-reference with structured metadata sources if available');
    }
    
    if (issues.some(issue => issue.type === 'context_mismatch')) {
      recommendations.push('Review entity relevance to document domain and themes');
    }
    
    if (metrics.mentionScore > 0.8 && metrics.contextScore < 0.5) {
      recommendations.push('High mention frequency with low context score - possible false positive');
    }
      ...
      for (let i = 0; i < group.length; i++) {
        for (let j = i + 1; j < group.length; j++) {
          ...
          if (entity1.name && entity2.name) {
            const similarity = this.calculateNameSimilarity(entity1.name, entity2.name);
            ...
            if (similarity > 0.8) {
              group[i].issues.push({
                ...
                suggestion: 'Consider merging or deduplicating similar entities'
              });
            }
          }
        }
  ...
  private identifyCrossTypeDuplicates(profiles: EntityQualityProfile[]): void {
    ...
    crossTypeIssues.forEach((duplicateProfiles, normalizedName) => {
      ...
      const typeConflictSeverity = this.assessCrossTypeConflictSeverity(Array.from(entityTypes));
      
      // Add issues to lower-quality entities in the conflict
      const sortedProfiles = duplicateProfiles.sort((a, b) => b.metrics.overallScore - a.metrics.overallScore);
      
      for (let i = 1; i < sortedProfiles.length; i++) {
        const conflictingTypes = Array.from(entityTypes).filter(t => t !== sortedProfiles[i].entity.kind);
        ...
        sortedProfiles[i].issues.push({
          ...
          suggestion: this.getCrossTypeResolutionSuggestion(Array.from(entityTypes), normalizedName)
        });
      }
    });
  }
  ...
  private normalizeEntityName(name: string): string {
    return name.toLowerCase()
      ...
      .replace(/\b(the|inc|corp|ltd|llc|co|company|corporation|incorporated)\b/g, '') // Remove common company suffixes
      .trim();
  }
   ...
   */
  private assessCrossTypeConflictSeverity(types: EntityKind[]): 'critical' | 'major' | 'minor' | 'warning' {
    ...
    if (typeSet.has('person') && (typeSet.has('organization') || typeSet.has('product'))) {
      return 'major';
    }
    
    // Minor conflicts: technology vs component vs product
    if ((typeSet.has('technology') || typeSet.has('component')) && typeSet.has('product')) {
      return 'minor';
    }
   ...
   */
  private getCrossTypeResolutionSuggestion(types: EntityKind[], entityName: string): string {
    const typeSet = new Set(types);
