...
export class QueryTransformationEngine {
  ...
  async transformQuery(
    ...
    console.log(
      `🎯 Query intent: ${intent.type} (complexity: ${intent.complexity})`
    );
    ...
    console.log(
      `✅ Query transformation complete: ${optimizedSearchQueries.length} search strategies generated`
    );
  ...
  private determineComplexity(
    ...
    const conjunctionCount = (
      query.match(/\b(and|or|but|while|also|furthermore|however)\b/gi) || []
    ).length;
  ...
  private async normalizeEntities(
    query: string
  ): Promise<{ query: string; transformed: boolean }> {
  ...
  private rewriteCausal(query: string): RewrittenQuery[] {
    ...
    const hasCausalTerm = causalTerms.some(term =>
   ...
   */
  private generateGenericRewrites(
    query: string,
    intent: QueryIntent
  ): RewrittenQuery[] {
    const rewrites: RewrittenQuery[] = [];
    ...
    const generalQuery = query.replace(
      /\b(specific|particular|exact|precise)\b/gi,
      ''
    );
  ...
  private async decomposeQuery(
    ...
    const parts = originalQuery.split(
      /\b(and|or|but|also|furthermore|however|while)\b/i
    );
  ...
  private async extractTopics(query: string): Promise<string[]> {
    ...
    if (topics.length === 0) {
      ...
      for (let i = 0; i < words.length - 1; i++) {
        ...
        if (
          /^[A-Z]/.test(phrase) ||
          /\b(technology|system|method|approach)\b/i.test(phrase)
        ) {
          topics.push(phrase);
        }
      }
   ...
   */
  private determineExpectedAnswerType(
    query: string
  ): SubQuery['expectedAnswerType'] {
    const queryLower = query.toLowerCase();

    if (
      queryLower.includes('who') ||
      queryLower.includes('person') ||
      queryLower.includes('inventor')
    ) {
      return 'entity';
    }
    ...
    if (
      ...
      queryLower.includes('examples') ||
      queryLower.includes('types')
    ) {
      return 'list';
    }
  ...
  private generateQueryPlan(subQueries: SubQuery[]): QueryPlan {
    ...
    // Sort by priority
    const sortedQueries = [...subQueries].sort(
  ...
  private async generateOptimizedSearchQueries(
    ...
    console.log(
      `🎯 Generated ${searchQueries.length} optimized search queries`
    );
