...
export const DOCUMENT_TYPES: Record<string, DocumentTypeDefinition> = {
  ...
  'patent': {
    ...
    richMetadataFields: ['inventors', 'assignees', 'originalAssignee', 'jurisdiction', 'classification', 'claimCount'],
    hasContentChunks: true,
    hasMetadataChunks: true,
    queryIndicators: ['patent', 'invention', 'inventor', 'assignee', 'claim', 'USPTO', 'EPO'],
    queryBoost: 1.2
  },
  ...
  'paper': {
    ...
    richMetadataFields: ['authorsAffiliations', 'venue', 'keywords', 'citationCount', 'impactFactor'],
    hasContentChunks: true,
    hasMetadataChunks: true,
    queryIndicators: ['paper', 'research', 'study', 'journal', 'conference', 'author'],
    queryBoost: 1.1
  },
  ...
  'article': {
    ...
    richMetadataFields: ['authorsAffiliations', 'venue', 'keywords', 'citationCount'],
    hasContentChunks: true,
    hasMetadataChunks: true,
  ...
  'thesis': {
    ...
    richMetadataFields: ['authorsAffiliations', 'university', 'degree', 'advisor'],
    hasContentChunks: true,
    hasMetadataChunks: true,
  ...
  'guideline': {
    ...
    richMetadataFields: ['society', 'specialty', 'evidenceLevel', 'recommendations'],
    hasContentChunks: true,
    hasMetadataChunks: true,
  ...
  'book': {
    ...
    richMetadataFields: ['authorsAffiliations', 'publisher', 'edition', 'pages'],
    hasContentChunks: true,
    hasMetadataChunks: true,
  ...
  'press-article': {
    ...
    richMetadataFields: ['outlet', 'journalist', 'oem', 'model', 'displaySpecs', 'leiaFeatures'],
    hasContentChunks: true,
    hasMetadataChunks: true,
 ...
 */
export function getDocumentType(docType: string): DocumentTypeDefinition | null {
 ...
 */
export function getDocumentTypesForPersona(persona: 'technical' | 'legal' | 'business' | 'medical' | 'general'): string[] {
  switch (persona) {
    case 'technical':
      return ['patent', 'paper', 'article', 'thesis', 'book', 'note', 'press', 'press-article'];
    case 'legal':
...
export function classifyQueryByDocumentType(query: string): {
  ...
  const confidence = matches.length > 0 ? Math.min(matches[0].score / 3, 1.0) : 0;
...
export function validateDocumentMetadata(
  ...
  const hasIdentifier = definition.identifierFields.some(field =>
    metadata.identifiers && metadata.identifiers[field]
  );

  if (!hasIdentifier && definition.identifierFields.length > 0) {
    errors.push(`At least one identifier is required: ${definition.identifierFields.join(', ')}`);
  }
