...
const DAVID_EXPERTISE_DOMAINS = {
  ...
  optics: ['optics', 'photonics', 'lightfield', 'diffractive', 'holographic', 'lens', 'display', 'backlight'],
  displays: ['3d display', 'glasses-free', 'stereoscopic', 'autostereoscopic', 'lcd', 'switchable'],
  ai: ['artificial intelligence', 'machine learning', 'depth estimation', 'computer vision', 'neural', 'ai'],
  spatialai: ['spatial ai', 'spatial intelligence', 'physical ai', 'embodied ai', '3d ai'],

  // Business domains
  companies: ['leia', 'immersity', 'hp labs', 'dimenco'],
  entrepreneurship: ['startup', 'founder', 'cto', 'fundraising', 'oem', 'partnership'],

  // Research domains
  research: ['patent', 'paper', 'publication', 'research', 'invention', 'ieee', 'arxiv'],
  people: ['david fattal', 'david', 'fattal'],

  // Related technical areas
  tech: ['quantum computing', 'nanophotonics', 'semiconductor', 'silicon', 'manufacturing']
};
  ...
  for (const pattern of ALWAYS_RAG_PATTERNS) {
    if (pattern.test(normalizedQuery)) {
      return {
        ...
        confidence: 0.95,
        reasoning: 'Direct reference to David\'s work or specific research content',
  ...
  if (domainMatches.length === 0) {
    ...
    if (isGeneralTechnicalQuery(normalizedQuery)) {
      return {
        ...
        confidence: 0.3,
        reasoning: 'General technical query that might benefit from corpus knowledge',
        domains: []
      };
    } else {
      return {
...
function calculateDomainScore(domains: string[], query: string): number {
  ...
  const questionWords = ['what', 'how', 'why', 'when', 'who', 'where', 'explain', 'tell', 'describe'];
  ...
  const analyticalWords = ['difference', 'compare', 'versus', 'better', 'advantage', 'disadvantage'];
...
function isGeneralTechnicalQuery(query: string): boolean {
  const generalTechWords = [
    'algorithm', 'software', 'hardware', 'technology', 'system', 'method',
    'process', 'technique', 'approach', 'solution', 'implementation',
    'framework', 'platform', 'architecture', 'design', 'development'
  ];
...
export function shouldUseRAGForQuery(query: string): boolean {
  ...
  console.log(`🎯 Query classification: ${result.classification} (confidence: ${result.confidence.toFixed(2)})`);
