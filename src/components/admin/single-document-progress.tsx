...
interface ProcessingStage {
  id: 'upload' | 'analysis' | 'chunking' | 'entities' | 'done';
  label: string;
  icon: string;
  description: string;
  completed: boolean;
...
export function SingleDocumentProgress({ jobId, onComplete }: SingleDocumentProgressProps) {
  const [stages, setStages] = useState<ProcessingStage[]>([
    ...
    { id: 'entities', label: 'Entity Extraction', icon: '🕷️', description: 'Extracting entities and updating knowledge graph', completed: false, active: false },
    { id: 'done', label: 'Complete', icon: '✅', description: 'Ready for search and chat', completed: false, active: false }
  ]);
    ...
    setStages(prevStages => prevStages.map(stage => {
      ...
      switch (stage.id) {
        ...
        case 'analysis':
          ...
          if (completed) details = 'Document analysis complete ✓';
          else if (active) details = 'Analyzing document structure and content...';
          break;
        ...
        case 'entities':
          ...
          active = progress > 0.5 && progress <= 0.9;
          if (completed && job.results?.entitiesExtracted) {
            details = `${job.results.entitiesExtracted} entities extracted and added to knowledge graph ✓`;
          } else if (active) {
            details = 'Extracting entities and updating knowledge graph...';
          }
          break;
