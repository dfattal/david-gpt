...
export function SearchAnalyticsDashboard({
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await fetch(
        `/api/admin/search-analytics?timeframe=${timeframe}`
      );
      if (!response.ok) {
  ...
  const exportAnalytics = () => {
    window.open(
      `/api/admin/search-analytics?timeframe=${timeframe}&format=csv`,
      '_blank'
    );
  };
    ...
    <div className={`p-6 space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg border p-6">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {data.recentActivity.queriesLastHour}
          </p>
      ...
      <div className="bg-white rounded-lg border p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">
              {data.performanceBenchmarks.sqlTier.performanceMet.toFixed(1)}%
            </p>
            <Badge
              className={getPerformanceBadge(
                data.performanceBenchmarks.sqlTier.performanceMet,
                80
              )}
            >
              Target: 80%
            ...
            <p className="text-sm text-gray-500 mt-1">
              {data.performanceBenchmarks.sqlTier.under200ms}/
              {data.performanceBenchmarks.sqlTier.totalQueries} queries
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">
              {data.performanceBenchmarks.vectorTier.performanceMet.toFixed(1)}%
            </p>
            <Badge
              className={getPerformanceBadge(
                data.performanceBenchmarks.vectorTier.performanceMet,
                80
              )}
            >
              Target: 80%
            ...
            <p className="text-sm text-gray-500 mt-1">
              {data.performanceBenchmarks.vectorTier.under1000ms}/
              {data.performanceBenchmarks.vectorTier.totalQueries} queries
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">
              {data.performanceBenchmarks.overallSuccess.rate.toFixed(1)}%
            </p>
            <Badge
              className={getPerformanceBadge(
                data.performanceBenchmarks.overallSuccess.rate,
                95
              )}
            >
              Target: 95%
              ...
              {data.queryPatterns.slice(0, 10).map((pattern, index) => (
                <tr key={index}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {pattern.query.length > 50
                      ? pattern.query.substring(0, 50) + '...'
                      : pattern.query}
                  </td>
